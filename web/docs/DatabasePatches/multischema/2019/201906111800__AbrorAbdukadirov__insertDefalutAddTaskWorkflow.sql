...
BEGIN
  ...
  if exists (select id from "anv".companyEmailNotificationSettings where category = 'CATEGORY_PM' and notificationname ='TASK_ADD_NOTIFICATION' limit 1)
  then
    insert into "anv".workflowrule (name, executioncriteria, module, active) values ('Default Task add', '_WORKFLOW_EXECUTION_CRITERIA_CREATE', '_WORKFLOW_MODULE_TASK', false) returning id into addWorkflowID;
  else
    insert into "anv".workflowrule (name, executioncriteria, module, active) values ('Default Task add', '_WORKFLOW_EXECUTION_CRITERIA_CREATE', '_WORKFLOW_MODULE_TASK', false)  returning id into addWorkflowID;
  end if;
