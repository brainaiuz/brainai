DROP FUNCTION IF EXISTS "0".createDefaultDeleteTaskWorkflow();
CREATE OR REPLACE FUNCTION "0".createDefaultDeleteTaskWorkflow()
  RETURNS INTEGER AS
$body$
DECLARE
...
BEGIN
  ...
  if exists (select id from "0".companyEmailNotificationSettings where category = 'CATEGORY_PM' and notificationname ='TASK_DELETE_NOTIFICATION' limit 1)
  then
    insert into "0".workflowrule (name, executioncriteria, module, active) values ('Default Task delete', '_WORKFLOW_EXECUTION_CRITERIA_REMOVE', '_WORKFLOW_MODULE_TASK', false) returning id into addWorkflowID;
  else
    insert into "0".workflowrule (name, executioncriteria, module, active) values ('Default Task delete', '_WORKFLOW_EXECUTION_CRITERIA_REMOVE', '_WORKFLOW_MODULE_TASK', false)  returning id into addWorkflowID;
  end if;
  ...
  VALUES (FALSE, NULL, '${assignees_emails}', NULL, 'Delete Task', '', '', NULL, addWorkflowID, 0, 'TRIGGER_TIME', 'MINUTES', FALSE,
...
ALTER FUNCTION "0".createDefaultDeleteTaskWorkflow() OWNER TO wfmtest;

UPDATE company SET selectFunctioncolumn =(SELECT "0".createDefaultDeleteTaskWorkflow()) where  id=(select id from company limit 1);
