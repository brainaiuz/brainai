...
delete from "anv".reference  where code = '_WORKFLOW_MODULE_GDN';
insert into "anv".reference(code, deleted, isremovable, name, shared, sorder, parentid, isactive)
							values('_WORKFLOW_MODULE_GDN', false, true, 'GDN', true,
...
delete from "0".reference  where code = '_WORKFLOW_MODULE_GDN';
...
delete from "model"  where formid = 'GDN_FORM';
insert into model(active, formid, title, viewname) values(true, 'GDN_FORM', 'GDN Form', 'GDN');
