DROP FUNCTION IF EXISTS "anv".createDefaultTaskWorkflow();
CREATE OR REPLACE FUNCTION "anv".createDefaultTaskWorkflow()
  RETURNS INTEGER AS
$body$
DECLARE
  eidtWorkflowID INTEGER;

BEGIN
  if exists (select id from "anv".companyEmailNotificationSettings where category = 'CATEGORY_PM' and notificationname ='TASK_UPDATE_NOTIFICATION' limit 1)
  then
    insert into "anv".workflowrule (name, executioncriteria, module, active) values ('Default Task edit', '_WORKFLOW_EXECUTION_CRITERIA_EDIT', '_WORKFLOW_MODULE_TASK', false) returning id into eidtWorkflowID;
  else
    insert into "anv".workflowrule (name, executioncriteria, module, active) values ('Default Task edit', '_WORKFLOW_EXECUTION_CRITERIA_EDIT', '_WORKFLOW_MODULE_TASK', false)  returning id into eidtWorkflowID;
  end if;
  ...
  VALUES (FALSE, NULL, '${assignments_emails}', NULL, 'Update Task', '', '', NULL, eidtWorkflowID, 0, 'TRIGGER_TIME', 'MINUTES', FALSE,
          '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
</head><body>
<p>Please be informed that ${modifier_name} updated ${name} task on ${updated_date}.</p>
...
ALTER FUNCTION "anv".createDefaultTaskWorkflow() OWNER TO wfmtest;

UPDATE company SET selectFunctioncolumn =(SELECT "anv".createDefaultTaskWorkflow()) where id||'' = replace('"anv"', '"', '');
