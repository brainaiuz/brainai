DROP FUNCTION IF EXISTS "anv".createDefaultCompleteTaskWorkflow();
CREATE OR REPLACE FUNCTION "anv".createDefaultCompleteTaskWorkflow()
  RETURNS INTEGER AS
$body$
DECLARE
  workflowID INTEGER;
...
BEGIN
  ...
  if exists (select id from "anv".companyEmailNotificationSettings where category = 'CATEGORY_PM' and notificationname ='TASK_COMPLETED_NOTIFICATION' limit 1)
  then
    insert into "anv".workflowrule (name, executioncriteria, module, active) values ('Default Task Completed', '_WORKFLOW_EXECUTION_CRITERIA_EDIT', '_WORKFLOW_MODULE_TASK', false) returning id into workflowID;
    insert into "anv".workflow_condition (column_id, operand, conditionid, workflow, value, conditions)
    values ('STATUS', 'EQUAL', 1, workflowID, (select id || '@' || name || '@' || 'undefined' as code from "anv".reference
                                               where code = 'COMPLETED'
                                                 and parentid = (select id from "anv".reference where code = '_TASK_STATUS') limit 1), workflowID);
  else
    insert into "anv".workflowrule (name, executioncriteria, module, active) values ('Default Task Completed', '_WORKFLOW_EXECUTION_CRITERIA_EDIT', '_WORKFLOW_MODULE_TASK', false)  returning id into workflowID;
    insert into "anv".workflow_condition (column_id, operand, conditionid, workflow, value, conditions)
    values ('STATUS', 'EQUAL', 1, workflowID, (select id || '@' || name || '@' || 'undefined' as code from "anv".reference
  ...
  VALUES (FALSE, NULL, '${assignees_emails}', NULL, 'Task Completed', '', '', NULL, workflowID, 0, 'TRIGGER_TIME', 'MINUTES', FALSE,
...
<body>
<p>Please be informed that ${modifier_name} completed ${name} task on ${updated_date}.</p>
...
<p>Click <a href="${link}" title="View task">here</a> to view the task</p>
...
ALTER FUNCTION "anv".createDefaultCompleteTaskWorkflow() OWNER TO wfmtest;

UPDATE company SET selectFunctioncolumn =(SELECT "anv".createDefaultCompleteTaskWorkflow()) where id||'' = replace('"anv"', '"', '');
