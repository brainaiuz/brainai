...
public abstract class AbstractSchemaMigrator implements SchemaMigrator {
    ...
    public void doMigration(Metadata metadata, ExecutionOptions options, TargetDescriptor targetDescriptor) {
        ...
        if ( !targetDescriptor.getTargetTypes().isEmpty() ) {
            ...
            try {
                final DatabaseInformation databaseInformation = getDatabaseInformation(
    ...
    protected DatabaseInformation getDatabaseInformation(
        ...
        return Helper.buildDatabaseInformation(
                serviceRegistry,
                ddlTransactionIsolator,
                sqlStringGenerationContext,
                tool
    ...
    private void performMigration(
        ...
        for ( AuxiliaryDatabaseObject auxiliaryDatabaseObject : database.getAuxiliaryDatabaseObjects() ) {
            if ( auxiliaryDatabaseObject.appliesToDialect( dialect ) ) {
                applySqlStrings(
                        true,
                        dialect.getAuxiliaryDatabaseObjectExporter().getSqlDropStrings(auxiliaryDatabaseObject, metadata, sqlStringGenerationContext),
                        formatter,
                        options,
                        targets
                );
            ...
            if ( schemaFilter.includeNamespace( namespace ) ) {
                for ( Sequence sequence : namespace.getSequences() ) {
                    ...
                    if ( sequenceInformation == null ) {
                        applySqlStrings(false, dialect.getSequenceExporter().getSqlCreateStrings(sequence, metadata, sqlStringGenerationContext), formatter, options, targets);
                    }
                }
            }
        }
