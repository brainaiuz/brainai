...
public class SimpleGrid extends AdvancedFlexTable implements Resizable {
    ...
    public void setHeaderWidget(int column, Widget widget) {
        ...
        th.addClassName("table-header-column");
        if (isResizable()) {
     ...
     */
    public void setColumnWidth(int column, double size, boolean... pixel) {
        String unit = pixel[0] ? "px" : "%";
        if (isResizable()) {
            if (size < 0)
                size = 0;
            Element th = getThElement(column);
            if (th != null)
                DOM.setStyleAttribute(th, "width", size + unit);

            Element tdFooter = getFooterTdElement(column);
            if (tdFooter != null)
                DOM.setStyleAttribute(tdFooter, "width", size + unit);

            if (getRowCount() > 0) {
                HTMLTable.CellFormatter formatter = getBodyTable().getCellFormatter();
                Element td = formatter.getElement(0, column);
                if (th != null)
                    DOM.setStyleAttribute(td, "width", th.getClientWidth() + unit);
                else
                    DOM.setStyleAttribute(td, "width", size + unit);
            }
        }
    }
     ...
     */
    public AdvancedFlexTable getHeaderTable() {
        if (headerTable == null)
