 ...
 */
@Transactional
public class ChartOfAccountEventListenerImpl extends CustomBusinessEventListenerAdapter {

    public static WfmType<EdsAccount> TYPE = new WfmType<>(EventTypes.chartOfAccountEventListener);
    @Autowired
    ...
    public void onAddEvent(EdsBusinessEvent event) {
        ...
        EdsAccount chartOfAccount = accountingManager.get(event.getEntityID());

        if (!event.isMyUpdatesItemAdd()) {
            try {
                EdsMyUpdate myUpdate = myUpdateManager.registerChartOfAccountAddUpdate(chartOfAccount, creator, event.getTime());
    ...
    public void onEditEvent(EdsBusinessEvent event) {
        ...
        if (!event.isMyUpdatesItemEdit()) {
            try {
                EdsMyUpdate myUpdate = myUpdateManager.registerChartOfAccountEditUpdate(chartOfAccount, creator, event.getTime());
                myUpdate.setSuperUser(event.isSuperUser());
    ...
    public void onDeleteEvent(EdsBusinessEvent event) {
        ...
        if (!event.isMyUpdatesItemEdit()) {
            try {
                EdsMyUpdate myUpdate = myUpdateManager.registerChartOfAccountDeleteUpdate(event.getEntityID(), event.getCustomStringField(), creator, event.getTime());
                myUpdate.setSuperUser(event.isSuperUser());
                event.setMyUpdatesItemEdit(true);
            } catch (Exception ex) {
                event.setMyUpdatesItemEdit(false);
