...
public class EmailAddressValidator {
    ...
    private static final Pattern EMAIL_PATTERN = Pattern.compile("^([A-Za-z0-9-_&*=+?'^{}~]+([.]{1})?){0,}" +
            "[A-Za-z0-9-_&*=+?'^{}~]{1}[@]{1}(([A-Za-z0-9]+[-.]?){0,}[A-Za-z0-9]{1}){1,}[.]{1}[A-Za-z]{2,20}$");

    public static String getHost(String email) {
        int index;
        if (email != null && !"".equals(email) && (index = email.indexOf("@")) > 0) {
            return email.trim().substring(index + 1);
        }
        return null;
    }
...
//        return checkHostMX(email) != null;
        return checkEmail(email); //check for host has been removed, uncomment above line to make it work
    }
