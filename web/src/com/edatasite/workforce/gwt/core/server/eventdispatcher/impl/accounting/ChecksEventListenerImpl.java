 ...
 */
@Transactional
public class ChecksEventListenerImpl extends CustomBusinessEventListenerAdapter {

    public static WfmType<EdsBankCheck> TYPE = new WfmType<>(EventTypes.checkEventListener);
    @Autowired
    ...
    public void onAddEvent(EdsBusinessEvent event) {
        ...
        EdsBankCheck invoice = bankCheckManager.get(event.getEntityID());
        EdsUser creator = userManager.get(event.getSourceID());

        if (!event.isMyUpdatesItemAdd()) {
            try {
                EdsMyUpdate myUpdate = myUpdateManager.registerCheckAddUpdate(invoice, creator, event.getTime());
    ...
    public void onEditEvent(EdsBusinessEvent event) {
        ...
        EdsBankCheck bankCheck = bankCheckManager.get(event.getEntityID());
        EdsUser creator = userManager.get(event.getSourceID());

        if (!event.isMyUpdatesItemEdit()) {
            try {
                EdsMyUpdate myUpdate = myUpdateManager.registerCheckEditUpdate(bankCheck, creator, event.getTime());
                myUpdate.setSuperUser(event.isSuperUser());
    ...
    public void onDeleteEvent(EdsBusinessEvent event) {
        ...
        if (!event.isMyUpdatesItemDelete()) {
            try {
                EdsMyUpdate myUpdate = myUpdateManager.registerCheckDeleteUpdate(event.getEntityID(), itemName, creator, event.getTime());
                myUpdate.setSuperUser(event.isSuperUser());
