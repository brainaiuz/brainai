...
public class AnnualAllowanceBalanceReportExcelHandler extends BaseExcelHandler implements Constants {
    private static final Logger log = LoggerFactory.getLogger(AnnualAllowanceBalanceReportExcelHandler.class);
    ...
    protected HSSFWorkbook getWorkBook(Object object) {
        ...
        ListResult<LeaveBalanceReport> reportDataList = availabilityServiceLocal.getAnnnualLeaveBalanceReportProrataBased(filterParametrs);
        ...
        mapColumnHeader.put(LeaveBalanceReport.OPENING_DATE, commonLocalizer.localize(PdfLocalizationName.openingBalanceDate));
        mapColumnHeader.put(LeaveBalanceReport.OPENING_BALANCE, commonLocalizer.localize(PdfLocalizationName.openingBalanceDays));
        mapColumnHeader.put(LeaveBalanceReport.EFFECTIVE_STARTDATE, commonLocalizer.localize(PdfLocalizationName.effectiveStartDate));
        mapColumnHeader.put(LeaveBalanceReport.EFFECTIVE_ENDDATE, commonLocalizer.localize(PdfLocalizationName.effectiveEndDate));
        ...
        mapColumnHeader.put(LeaveBalanceReport.UNPAID_DAYS, commonLocalizer.localize(PdfLocalizationName.unpaidDays));
        mapColumnHeader.put(LeaveBalanceReport.WORKED_DAYS, "Worked days");
        mapColumnHeader.put(LeaveBalanceReport.CURRENT_BALANCE, commonLocalizer.localize(PdfLocalizationName.currentBalance));
        ...
        try {
            ...
            for (LeaveBalanceReport item : reportDataList.getList()) {
                ...
                for (int ii = 0; ii < header.size(); ii++) {
                    ...
                    if (LeaveBalanceReport.OPENING_DATE.equals(header.get(ii))) {
                        cell.add(header.indexOf(LeaveBalanceReport.OPENING_DATE), (item.getOpeningDate() != null ? ServerUtils.shortDateFormat(item.getOpeningDate().getNonConvertedDate(), user) : ""));
                    }
                    if (LeaveBalanceReport.OPENING_BALANCE.equals(header.get(ii))) {
                        cell.add(header.indexOf(LeaveBalanceReport.OPENING_BALANCE), String.valueOf(item.getOpeningBalance() != null ? item.getOpeningBalance() : 0.0));
                    }

                    if (LeaveBalanceReport.EFFECTIVE_STARTDATE.equals(header.get(ii))) {
                        cell.add(header.indexOf(LeaveBalanceReport.EFFECTIVE_STARTDATE), item.getEffectiveStartDate() != null ? ServerUtils.shortDateFormat(item.getEffectiveStartDate().getNonConvertedDate(), user) : "");
                    }
                    if (LeaveBalanceReport.EFFECTIVE_ENDDATE.equals(header.get(ii))) {
                        cell.add(header.indexOf(LeaveBalanceReport.EFFECTIVE_ENDDATE), item.getEffectiveEndDate() != null ? ServerUtils.shortDateFormat(item.getEffectiveEndDate().getNonConvertedDate(), user) : "");
                    ...
                    if (LeaveBalanceReport.UNPAID_DAYS.equals(header.get(ii))) {
                        cell.add(header.indexOf(LeaveBalanceReport.UNPAID_DAYS), getMoneyFormat(item.getUnpaidDays()));

                    }
                    if (LeaveBalanceReport.WORKED_DAYS.equals(header.get(ii))) {
                        cell.add(header.indexOf(LeaveBalanceReport.WORKED_DAYS), getMoneyFormat(item.getWorkedDays()));
                ...
                for (int k = 0; k < header.size(); k++) {
                    ...
                    if (LeaveBalanceReport.OPENING_BALANCE.equals(columnCode) ||
                            LeaveBalanceReport.LEAVE_ALLOWANCE_DAYS.equals(columnCode) ||
                            LeaveBalanceReport.TAKEN_DAYS.equals(columnCode) ||
                            LeaveBalanceReport.UNPAID_DAYS.equals(columnCode) ||
                            LeaveBalanceReport.WORKED_DAYS.equals(columnCode) ||
                            LeaveBalanceReport.CURRENT_BALANCE.equals(columnCode)) {
                        dataType = ExcelData.BIG_DECIMAL;
                    }
    ...
    protected void setFileName() {
        filename = "Annual_Allowance_Balance_Report_List_" + dateFormat(new Date());
    }
}
