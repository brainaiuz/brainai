 ...
 */
public class SaleQuoteListPDFHandler extends BaseInvoiceListPDFHandler {
    @Autowired
    ...
    protected InvoiceList getInvoiceData(ListingFilterParameter fp, ListLoadConfig config) {
        return invoiceCircularResolver.getSaleQuoteData(fp);
    }
    ...
    public String getFileName() {
        return accountingLocalizer.localize(PdfLocalizationName.saleQuotesList);
    }
    ...
    protected String getStatusForShow(String status, NewInvoice data) {
        ...
        if (APPROVE.equals(data.getStatusCode())) {
            return commonLocalizer.localizeAccounting(PdfLocalizationName.approvedByManager);
        } else if (MANAGER_REJECT.equals(data.getStatusCode())) {
            return commonLocalizer.localizeAccounting(PdfLocalizationName.rejectedByManager);
        } else if (CLIENT_APPROVE.equals(data.getStatusCode())) {
            return commonLocalizer.localizeAccounting(PdfLocalizationName.approvedByClient);
        } else if (REJECT.equals(data.getStatusCode())) {
            return commonLocalizer.localizeAccounting(PdfLocalizationName.rejectedByClient);
        } else if (SUBMITTED_TO_MANAGER.equals(data.getStatusCode())) {
            return commonLocalizer.localizeAccounting(PdfLocalizationName.waitingForManagerApprove);
        } else {
    ...
    protected ITextGenericPdfData buildPdfDocumentCustomise(Object dataClass, EdsCompany company, boolean hasPhantom) {
        ...
        columnHeaderMap.put(InvoiceList.INVOICE_NUMBER, new CellData(accountingLocalizer.localize(PdfLocalizationName.Qnumber, "Number"), Element.ALIGN_LEFT));
        ...
        columnHeaderMap.put(InvoiceList.CLIENT, new CellData(commonLocalizer.localize(PdfLocalizationName.customer), Element.ALIGN_LEFT));
        ...
        columnHeaderMap.put(InvoiceList.BASE_TOTAL, new CellData(commonLocalizer.localize(PdfLocalizationName.total), Element.ALIGN_LEFT));
        ...
        columnHeaderMap.put(InvoiceList.PO_NUMBER, new CellData(commonLocalizer.localize(PdfLocalizationName.poNumber), Element.ALIGN_LEFT));
        columnHeaderMap.put(InvoiceList.REFERENCE, new CellData(commonLocalizer.localize(PdfLocalizationName.reference), Element.ALIGN_LEFT));
        columnHeaderMap.put(InvoiceList.OPPORTUNITY_NUMBER, new CellData(commonLocalizer.localize(PdfLocalizationName.opportunity), Element.ALIGN_LEFT));
        ...
        for (NewInvoice item : quotes) {
            ...
            BigDecimal baseTotal = item.getTotal().setScale(calculationScale, RoundingMode.HALF_UP);
            ...
            if (panelTools.getColumnCodeName().contains(InvoiceList.INVOICE_NUMBER)) {
                mapColumn.put(InvoiceList.INVOICE_NUMBER, new CellData(item.getInvoiceNumber(), Element.ALIGN_LEFT));
            }
            ...
            if (panelTools.getColumnCodeName().contains(InvoiceList.CLIENT)) {
                mapColumn.put(InvoiceList.CLIENT, new CellData(item.getClientName(), Element.ALIGN_LEFT));
            }
            if (panelTools.getColumnCodeName().contains(InvoiceList.CREATOR)) {
                mapColumn.put(InvoiceList.CREATOR, new CellData(creator, Element.ALIGN_LEFT));
            }
            ...
            if (panelTools.getColumnCodeName().contains(InvoiceList.DUE_AMOUNT)) {
                mapColumn.put(InvoiceList.DUE_AMOUNT, new CellData(amount.toString(), Element.ALIGN_LEFT));
            }
            ...
            if (panelTools.getColumnCodeName().contains(InvoiceList.SUB_TOTAL)) {
                mapColumn.put(InvoiceList.SUB_TOTAL, new CellData(subTotal.toString(), Element.ALIGN_LEFT));
            }
            if (panelTools.getColumnCodeName().contains(InvoiceList.BASE_TOTAL)) {
                mapColumn.put(InvoiceList.BASE_TOTAL, new CellData(baseTotal.toString(), Element.ALIGN_LEFT));
            }
            if (panelTools.getColumnCodeName().contains(InvoiceList.TAX_TOTAL)) {
                mapColumn.put(InvoiceList.TAX_TOTAL, new CellData(taxTotal.toString(), Element.ALIGN_LEFT));
            }
            if (panelTools.getColumnCodeName().contains(InvoiceList.PO_NUMBER)) {
                mapColumn.put(InvoiceList.PO_NUMBER, new CellData(item.getPoNumber(), Element.ALIGN_LEFT));
            }
            ...
            if (panelTools.getColumnCodeName().contains(InvoiceList.OPPORTUNITY_NUMBER)) {
                mapColumn.put(InvoiceList.OPPORTUNITY_NUMBER, new CellData(item.getOpportunityNumber() != null ? item.getOpportunityNumber() : "", Element.ALIGN_LEFT));
            }
