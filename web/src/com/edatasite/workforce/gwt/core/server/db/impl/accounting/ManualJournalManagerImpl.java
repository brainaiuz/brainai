 ...
 */
@Repository("manualJournalManager")
public class ManualJournalManagerImpl extends BaseManager<EdsManualJournal> implements ManualJournalManager {
    public ManualJournalManagerImpl() {
        super(EdsManualJournal.class);
    }
...
//        sql.append(" GROUP BY mj.id ,mj.narration,mj.date,mj.status,mj.reference ");
    ...
    private void addManualJournalsFilterQuery(ListingFilterParameter fp, StringBuffer sql) {
        ...
        } else if (ServerUtils.hasPermission(PermissionConstants.ACCOUNTING_MANUAL_JOURNAL_SEE_OWN) && !getUser().hasRole(EdsRole.ADMIN_CODE)) {
            sql.append(" AND mj.creatorId = " + getUser().getObjectID());
        }
        if (fp.getProjectId() != null) {
            if (genericSettingsManager.isSettingsEnabled(GenericSettingsEnum.MANUAL_JOURNAL_PM_TO_HEAD_ENABLED)) {
                sql.append(" AND mj.projectid = " + fp.getProjectId());
            }
        }
    ...
    public void deleteJournalItems(Integer manualJournalID) {
        update("delete from EdsManualJournalItem mji where mji.manualJournal.objectID = ?", manualJournalID);
    }

    public void deleteManualJournalItemsByIds(String ids) {
        updateNative("delete from " + getCompanyId() + ".manualjournalitem where id in (" + ids + ")");
    }
    ...
    public List<TransactionAllocateItem> getManualTransactionsByCrmAccount(List<Integer> clientSupplierIds, boolean isClient, boolean fromApplyCredit, Date date, FindMatchFilterData filterData, Integer currencyID) {
        ...
        if (fromApplyCredit) {
            if (isClient) {
                sql.append("select mj.id as objectID, max(mj.currencyid) as currencyID, max(narration) as narration, max(mj.date) as date, sum(mji.credit) as amount, max(mj.reference) as reference, a.id accountID, ca.id as crmAccountID, ca.name as crmAccountName, mj.number as number from \"" + schema + "\".manualjournal mj ");
                ...
                sql.append("select mj.id as objectID, max(mj.currencyid) as currencyID, max(narration) as narration, max(mj.date) as date, sum(mji.debit) as amount, max(mj.reference) as reference, a.id accountID, ca.id as crmAccountID, ca.name as crmAccountName, mj.number as number from \"" + schema + "\".manualjournal mj ");
                sql.append("inner join \"" + schema + "\".manualjournalitem mji on mji.manualjournalid=mj.id ");
                sql.append("inner join ").append(getCompanyId()).append(".account a on a.id = mji.accountid ");
                sql.append("inner join ").append(getCompanyId()).append(".reference r on r.id = mj.overallStatus ");
                sql.append("inner join ").append(getCompanyId()).append(".crmaccount ca on ca.id = mji.client_or_supplier_id ");
        ...
        List<TransactionAllocateItem> result = new ArrayList<>(jdbcSpringManager.getSimpleJdbcTemplate().query(sql.toString(), BeanPropertyRowMapper.newInstance(TransactionAllocateItem.class)));
        return result;
    }
    ...
    public List<Object[]> getManualJournalList(ListingFilterParameter fp) {
        ...
        sql.append(" r.code as status, mj.reference, mj.recurringTemplate, mj.number,");
        if (genericSettingsManager.isSettingsEnabled(GenericSettingsEnum.MANUAL_JOURNAL_PM_TO_HEAD_ENABLED)) {
            sql.append("coalesce(pr.number||' -> ','')||pr.name project, ");
        } else {
            sql.append("array_to_string(array_agg(coalesce((case when  p.number is not null then null else '' end),  p.number || ' -> ') ||  (p.name)), ', ') project, ");
        }
        sql.append(" (u.firstname ||' '|| u.lastname) creator, cur.name, ap.exactapprover, mu.firstname || ' ' || mu.lastname, mj.creatorid");
        sql.append(" ");
        ...
        sql.append(" LEFT JOIN ").append(getCompanyId()).append(".project pr ON mj.projectid = pr.id ");
        ...
        sql.append(" LEFT JOIN ").append(getCompanyId()).append(".approvers ap on ap.id = mj.currentApprover ");
        sql.append(" LEFT JOIN ").append(getCompanyId()).append(".myuser mu on mu.id = ap.exactapprover ");
        sql.append(" LEFT JOIN ").append(getPublic()).append(".currency cur on mj.currencyid = cur.id ");
        if (genericSettingsManager.isSettingsEnabled(GenericSettingsEnum.ENABLE_MANUAL_ENTRY_ROLE)) {
            sql.append(" LEFT JOIN ").append(getCompanyId()).append(".role rl on mj.roleid = rl.id ");
            sql.append(" LEFT JOIN ").append(getCompanyId()).append(".myuser_role urole on rl.id = urole.roles_id ");
        }
        sql.append(" WHERE mj.deleted is not true AND (mj.overallStatus is null OR r.code!='" + EdsManualJournal.REVERSED + "')  ");
        if (genericSettingsManager.isSettingsEnabled(GenericSettingsEnum.ENABLE_MANUAL_ENTRY_ROLE)) {
            EdsUser edsUser = this.getUser();
            sql.append(" AND (mj.roleid is null or urole.roles_id in (" + edsUser.getRolesAsIntegersString() + "))");
        }

        addManualJournalsFilterQuery(fp, sql);

        sql.append(" GROUP BY mj.id ,mj.narration,mj.date,r.code,mj.recurringTemplate,mj.reference,mj.number,pr.number, pr.name, creator, cur.name, ap.exactapprover, mu.firstname, mu.lastname, mj.creatorid ");
        ...
        } else if (ManualJournalListItem.CURRENCY.equals(sortField)) {
            sql.append(" ORDER BY cur.name " + ascOrDesc);
        } else {
            sql.append(" ORDER BY mj.id desc ");
        }

        return findNativeLimited(sql + " offset " + fp.getStart(), fp.getLimit());
    }
    ...
    public TransactionAllocateItem getPaidManualTransaction(Integer objectID, Integer clientSupplierID, Integer accountID, boolean isReceivable) {
        ...
        sql.append("select mj.id objectID, mj.reference, mj.narration, mji.accountid, ").append(isReceivable ? "sum(coalesce(mji.debit,0)) amount " : "sum(coalesce(mji.credit,0)) amount ")
                .append(", mj.currencyid, mj.exchangerate, mj.date, mj.number as number  from ").append(getCompanyId()).append(".manualjournal mj \n");
        sql.append("inner join ").append(getCompanyId()).append(".manualjournalitem mji on mji.manualjournalid = mj.id \n");
...
//        TransactionAllocateItem item = jdbcSpringManager.getSimpleJdbcTemplate().queryForObject(sql.toString(), new HashMap<String, Object>(), BeanPropertyRowMapper.newInstance(TransactionAllocateItem.class));
    ...
    @Override
    public List<EdsManualJournalItem> getManualJournalItemList(ListingFilterParameter filterParametrs) {
        Map<String, Object> param = new HashMap<>();
        StringBuilder sql = new StringBuilder();

        sql.append("select distinct it.* from " + getCompanyId() + ".manualJournal mj ");
        sql.append("left join" + getCompanyId() + ".manualJournalItem it on it.manualjournalid=mj.id ");
        sql.append("left join" + getCompanyId() + ".project p on p.id = it.project_id ");
        sql.append("left join" + getCompanyId() + ".account a on a.id = it.accountid ");
        sql.append("left join ").append(getPublic()).append(".accounttype at on at.id = a.accountTypeId ");


        sql.append(" where " + ServerUtils.checkForDeleted("mj.deleted"));
        if (filterParametrs.getProjectId() != null && filterParametrs.getProjectId() != 0) {
            sql.append(" and (p.id=" + filterParametrs.getProjectId() + ") ");
        }

        return (List<EdsManualJournalItem>) findNative(sql.toString(), EdsManualJournalItem.class);
    }
