 ...
 */
@Transactional
public class CrmContactCategoryEventListener implements BusinessEventListener {
    public static WfmType<EdsContactCategory> TYPE = new WfmType<>(EventTypes.crmContactCategoryEventListener);
    @Autowired
    ...
    public void onAddEvent(EdsBusinessEvent event) {
        EdsContactCategory category = contactCategoryManager.get(event.getEntityID());
        EdsUser creator = userManager.get(event.getSourceID());
        if (!event.isMyUpdatesItemAdd()) {
            try {
                EdsMyUpdate myUpdate = myUpdateManager.registerContactCategoryAddUpdate(category, creator, event.getTime());
    ...
    public void onEditEvent(EdsBusinessEvent event) {
        ...
        if (!event.isMyUpdatesItemEdit()) {
            try {
                EdsMyUpdate myUpdate = myUpdateManager.registerContactCategoryEditUpdate(category, creator, event.getTime());
                myUpdate.setSuperUser(event.isSuperUser());
