 ...
 * Time: 12:11 PM
 */
public class NoteWidget extends SimplePanel implements Constants {
    protected VerticalPanel panel;
    protected TextArea2 textBox;
    ...
    public NoteWidget(Integer entityID, String entityType, HistoryListItem... notes) {
    ...
    private void drawInitialize() {
        init();
        WfmUiEventsBus.addWfmUiListener(WfmUiEventType.ON_NOTE_ADD, NoteWidget.this, (sender, args) -> {
            panel.clear();
            init();
        });
        WfmUiEventsBus.addWfmUiListener(WfmUiEventType.ON_NOTE_DELETE, NoteWidget.this, (sender, args) -> {
            panel.clear();
            init();
        });
        WfmUiEventsBus.addWfmUiListener(WfmUiEventType.ON_NOTE_EDIT, NoteWidget.this, (sender, args) -> {
            panel.clear();
            init();
        });
    }
    ...
    private void init() {
        ...
        if (panel == null || textBox == null) {
            ...
            textBox = new TextArea2(3000, true);
            textBox.getTextArea().addStyleName("noteTextArea");
            textBox.ensureDebugId("note-textArea");
            if (MEETING_MINUTES.equals(entityType)) {
                textBox.getTextArea().addStyleName("size190_70");
            } else {
                textBox.getTextArea().addStyleName("size470_70");
            }
                ...
                if ((event.getNativeEvent().getKeyCode() == 10 || event.getNativeEvent().getKeyCode() == KeyCodes.KEY_ENTER) && event.getNativeEvent().getCtrlKey()) {
                    if (textBox.getText() != null && !"".equals(textBox.getText().trim())) {
                        if (editingKey != null) {
                            ...
                            item.setComment(textBox.getText());
                            createNote(item, true);
                        } else {
                            createNote(new HistoryListItem(textBox.getText()), true);
                        }
                    }
                    textBox.setText("");
                }
...
//                buttonsPanel.add(radioButtonDiv);
        ...
        if (entityID != null && entityType != null) {
            allInOneService.getNotes(entityID, entityType, new AbstractAsyncCallback<ArrayList<HistoryListItem>>() {
                ...
                public void success(ArrayList<HistoryListItem> result) {
                    addNotesToWidget(result);
                }
            });
        }
    }

    private void addNotesToWidget(ArrayList<HistoryListItem> result) {
    ...
    public void createNote(final HistoryListItem note, boolean saving) {
        ...
        if (note.getObjectID() == null) {//this is new note...
            ...
            note.setEventDate(note.getEventDate() != null ? note.getEventDate() : new Date());
            note.setEmployee(Utils.isNullOrEmpty(note.getEmployee()) ? Utils.getUserFullName() : note.getEmployee());
            ...
            if ((MEETING_MINUTES.equals(entityType) || VACANCY.equals(entityType) || PLACEMENT.equals(entityType) ||
                ...
                BugReportService.App.get().addNote(note, new AbstractAsyncCallback<Integer>() {
                    ...
                    public void success(Integer result) {
                        ...
                        WfmUiEventsBus.fireWfmUiEvent(WfmUiEventType.ON_NOTE_ADD, result, NoteWidget.this);
                    }
                });
                ...
                allInOneService.saveCrmNote(entityType, entityID, note, new AbstractAsyncCallback<Integer>() {
                    ...
                    public void onSuccess(Integer result) {
                        ...
                        WfmUiEventsBus.fireWfmUiEvent(note.getObjectID() != null && !"".equals(note.getObjectID()) ? WfmUiEventType.ON_NOTE_EDIT : WfmUiEventType.ON_NOTE_ADD, result, NoteWidget.this);
                    }
                });
            }
        }
    ...
    private void edit(HistoryListItem note) {
        ...
        textBox.setText(note.getComment());
...
//                        WfmUiEventsBus.fireWfmUiEvent(WfmUiEventType.ON_NOTE_DELETE, result, NoteWidget.this);
                    }
                });
            }
        }
...
//        html.setWidth("100%");
    ...
    public ArrayList<HistoryListItem> getNewNotesToSave() {
        if (textBox != null && textBox.getText() != null && !"".equals(textBox.getText().trim())) {
            createNote(new HistoryListItem(textBox.getText()), true);
        }
    ...
    public void addConvertedNotes(ArrayList<HistoryListItem> notes) {
        addNotesToWidget(notes);
    }
}