...
public class RecurringCustomDeductionSummaryView extends CustomForm2 implements Colapse {
    ...
    public RecurringCustomDeductionSummaryView(Integer objectId) {
        super("summary");
...
//        setDescription(property.getSingular(payrollStrings.recurringDeductionCategory()));
    ...
    private void setData() {
        ...
        if (transferObject.getToDate() != null) {
            toDate.setHTML(DateUtils.format(transferObject.getToDate()));
        } else {
            toDate.setHTML(wfmStrings.na());
        }
    ...
    private void setButtons() {
        ...
        if (transferObject.getApprover() != null) {
            isApprove = Objects.equals(Utils.getUserID(), transferObject.getApprover().getId());
        }
...
//        declineButton = addButton(wfmStrings.decline(), clickEvent -> {
    ...
    protected void save(String status) {
        ...
        PayrollService.App.get().saveRecurringPaymentDeduction(transferObject, new AsyncCallback<TestRPC>() {
            ...
            public void onSuccess(TestRPC result) {
                LoadingPanel.loading(false);
                WfmUiEventsBus.fireWfmUiEvent(WfmUiEventType.ON_PAYROLL_RECURRING_PD_ADD, result, RecurringCustomDeductionSummaryView.this);
                closeTab();
            }
        });
    }
