 ...
 * <li>left arrow - shift left one stage</li>
 * <li>right arrow - shift right one stage</li>
 ...
 * line }</li> </ul>
 */
public class SliderBar extends FocusPanel implements ResizableWidget, HasValueChangeHandlers<Double> {

    private final NumberFormat numberFormat = NumberFormat.getFormat(",#0");
    private static NumberFormat extendedNumberFormat = NumberFormat.getFormat(",##0.00");
...
//    private Image knobImage = new Image();
     ...
     * Current value
     */
    private Element valueSpan;

    /**
     * The offset between the edge of the shell and the line.
     */
    private double lineLeftOffset = 0;
    ...
    public SliderBar(double minValue, double maxValue, LabelFormatter labelFormatter) {
        ...
        Element emElement = DOM.createElement("em");
        emElement.setClassName("emSlider");
        DOM.appendChild(knobElement, emElement);
        ...
        valueSpan = DOM.createInputText();
        DOM.appendChild(getElement(), valueSpan);
        DOM.setElementProperty(valueSpan, "className", "gwt-SliderBar-value form-control");
        DOM.setElementProperty(valueSpan, "maxLength", "4");
        valueSpan.setPropertyString("value", numberFormat.format(getCurrentValue()));
        DOM.setElementAttribute(valueSpan, "id", DOM.createUniqueId());

        DOM.sinkEvents(valueSpan, Event.ONCHANGE | Event.ONKEYUP | Event.ONKEYPRESS);

        setWidth("230px");
        DOM.sinkEvents(knobElement, Event.MOUSEEVENTS | Event.KEYEVENTS | Event.FOCUSEVENTS);
        DOM.sinkEvents(lineElement, Event.MOUSEEVENTS | Event.KEYEVENTS | Event.FOCUSEVENTS);
    }
    ...
    public double getCurrentValue() {
        curValue = Math.min(curValue, getMaxValue());
        return curValue;
    }
    ...
    public void onBrowserEvent(Event event) {
        ...
        if (enabled && DOM.eventGetType(event) > -1) {
            if (event.getTarget().equals(valueSpan)) {
                switch (DOM.eventGetType(event)) {
                    ...
                    case Event.ONKEYPRESS:
                        ...
                        break;
                    case Event.ONKEYUP:
                        DOM.releaseCapture(valueSpan);
                        String val = valueSpan.getPropertyString("value");
                        Double value = Double.valueOf(val);
                        if (value > getMaxValue()) {
                            value = getMaxValue();
                            //DOM.setElementAttribute(valueSpan, "value", numberFormat.format(value));
                            valueSpan.setNodeValue(numberFormat.format(value));
                        }
                        setCurrentValue(value);
                        break;
                    case Event.ONCHANGE:
                        DOM.releaseCapture(valueSpan);
                        val = valueSpan.getPropertyString("value");
                        value = Double.valueOf(val);
                        if (value > getMaxValue()) {
                            value = getMaxValue();
                            valueSpan.setPropertyString("value", numberFormat.format(value));
                        }
                        setCurrentValue(value);
                        break;
                }
                ...
                switch (DOM.eventGetType(event)) {
                    ...
                    case Event.ONBLUR:
                        keyTimer.cancel();
                        if (slidingMouse) {
                            DOM.releaseCapture(knobElement);
                            slidingMouse = false;
                            slideKnob(event);
    ...
    public void setEnabled(boolean enabled) {
        ...
        if (!enabled) {
            valueSpan.setAttribute("disabled", "disabled");
            knobElement.setAttribute("disabled", "disabled");
            DOM.setElementProperty(lineElement, "className", "gwt-SliderBar-line");
        } else {
            valueSpan.removeAttribute("disabled");
            knobElement.removeAttribute("disabled");
    ...
    public void setCurrentValue(double curValue, boolean fireEvent) {
        ...
        if (this.curValue != maxValue) {
            this.curValue -= remainder;
        }

        // Go to next stage if more than halfway there
        ...
        value = value.substring(0, (stepSize+"").length());
        valueSpan.setPropertyString("value", value);
        // Fire the onValueChange event
        if (fireEvent) {
            fireEvent(new OurValueChangeEvent(getCurrentValue()));
        }
    }
     ...
     * stage size, you will get a properly aligned "jumping" effect where the knob
     ...
     * <p/>
     * If you set the number of ticks equal to the total range divided by the stage
...
//        int knobWidth = DOM.getElementPropertyInt(knobElement, "offsetWidth");
        double knobLeftOffset = lineLeftOffset + (getKnobPercent() * lineWidth);
        DOM.setStyleAttribute(knobElement, "width", knobLeftOffset + "px");
    }
    ...
    private void drawLabels() {
        ...
        if (numLabels > 0) {
            ...
            for (int i = 0; i <= numLabels; i++) {
                ...
                int labelLeftOffset = (int) lineLeftOffset + (lineWidth * i / numLabels) - (labelWidth / 2);
                labelLeftOffset = Math.min(labelLeftOffset, (int) lineLeftOffset + lineWidth - labelWidth);
                labelLeftOffset = Math.max(labelLeftOffset, (int) lineLeftOffset);
...
//                DOM.setStyleAttribute(tick, "visibility", "hidden");
                ...
                int tickLeftOffset = (int) lineLeftOffset + (lineWidth * i / numTicks) - (tickWidth / 2);
                tickLeftOffset = Math.min(tickLeftOffset, (int) lineLeftOffset + lineWidth - tickWidth);
...
//                DOM.setStyleAttribute(tick, "visibility", "visible");
    ...
    private void startSliding(boolean highlight, boolean fireEvent) {
        if (highlight) {
            ...
            DOM.setElementProperty(knobElement, "className", "gwt-SliderBar-knob gwt-SliderBar-knob-sliding");
        }
    }
    ...
    static class OurValueChangeEvent extends ValueChangeEvent<Double> {
        /**
         * Creates a value change event.
         *
         * @param value the value
         */
        protected OurValueChangeEvent(Double value) {
            super(value);
        }
    }
