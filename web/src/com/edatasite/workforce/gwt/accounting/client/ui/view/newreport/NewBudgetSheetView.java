...
import com.edatasite.workforce.gwt.accounting.client.ui.view.newreport.BudgetSheet.NewBudgetSheetV2;
 ...
 */
public class NewBudgetSheetView extends View implements Colapse, AccountingConstants, FittedContent {

    private static final AccountingStrings accountingStrings = AccountingStrings.App.get();

    private NewBudgetSheetV2 newBudgetSheet;
    private NewBudgetSheet oldNewBudgetSheet;

    public NewBudgetSheetView() {
        super("budgetsheetView");
        setDescription(property.getSingular(wfmStrings.budgetManager()));
    }

    @Override
    protected Widget onInitialize() {
        LoadingPanel.loading(true);
        if (Utils.hasGenericAccess(GenericSettingsEnum.ENABLE_OLD_BUDGET_MANAGER)) {
            oldNewBudgetSheet = new NewBudgetSheet();
            add(oldNewBudgetSheet);
        } else {
            newBudgetSheet = new NewBudgetSheetV2();
            add(newBudgetSheet);
        }
        return null;
    }
    ...
    public String getIconStyle() {
        return "accountMark budgetsheet";
    }
