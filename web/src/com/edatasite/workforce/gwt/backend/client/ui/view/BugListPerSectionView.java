 ...
 */
public class BugListPerSectionView extends BaseListView {
    private static final BackendStrings backendStrings = BackendStrings.App.get();
    private ListingPanel<BugsPerEmployeesListItem> list;

    public BugListPerSectionView() {
        super("bugSection", backendStrings.summaryBySection());
    }

    public String getIconStyle() {
        return "backend bugListPerSecView";
    }

    protected Widget onInitialize() {
        list = new ListingPanel<>(ListPanelType.SummaryBySectionListPanel, drawColumns(), provider(), designer());

        list.getPdfVersion().setVisible(false);

        list.setExcelListener(clickEvent -> {
            String excelURL = CommandConstants.COMMON_URL + "/downloadBugListPerSectionViewExcel";
        ...
        BugListPerSectionQuickView bugSectionQuickView = new BugListPerSectionQuickView();

        list.setQuickViewPanel(bugSectionQuickView);
        super.setQuickViewPanel(bugSectionQuickView);
    ...
    private ColumnDefinitionConfig[] drawColumns() {
        ...
        columns[0] = new ColumnDefinitionConfig<BugsPerEmployeesListItem, String>(wfmStrings.section(), BugsPerEmployeesListItem.SECTION, 120) {
    ...
    private ListingRequestProvider<BugsPerEmployeesListItem> provider() {
        return (filterParametrs, callback) -> {
            ...
            backendService.getBugsPerSections(filterParametrs, new AbstractAsyncCallback<ListResult<BugsPerEmployeesListItem>>() {
