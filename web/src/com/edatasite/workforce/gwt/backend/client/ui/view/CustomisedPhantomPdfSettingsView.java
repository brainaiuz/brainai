 ...
 * Date: 04.10.2018 15:46
 */
public class CustomisedPhantomPdfSettingsView extends CustomForm implements Colapse {

    private static final BackendStrings backendStrings = BackendStrings.App.get();

    private TextBox name;
    private WfmDropdown type;
    ...
    private DataListBox pageFormat;
    private DataListBox orientation;
    ...
    private TextBox headerHeight;
    private TextBox footerHeight;
    ...
    public CustomisedPhantomPdfSettingsView(Integer companyId) {
        super("newpdftemplate", "Add Phantom Pdf " + companyId);
        this.companyId = companyId;
    }

    public CustomisedPhantomPdfSettingsView(Integer companyId, Integer objectId) {
        super("edit", "Edit Phantom Pdf " + companyId);
        this.companyId = companyId;
    ...
    protected Widget onInitialize() {
        ...
        header = new TextArea();
        header.setVisibleLines(8);

        body = new TextArea();
        body.setVisibleLines(8);

        footer = new TextArea();
        footer.setVisibleLines(8);
        ...
        pageFormat = new DataListBox();
        pageFormat.setItems(getPageFormats());
        pageFormat.setSelected(new SelectItem(2, "A4"));
        ...
        orientation = new DataListBox();
        orientation.setItems(getPageOrientations());
        orientation.setSelected(new SelectItem(1, "portrait"));

        marginTop = new TextBox();
        marginTop.setPlaceHolder("-30px");
        new KpiToolTip(marginTop, "Page Margin Top");
        marginRight = new TextBox();
        marginRight.setPlaceHolder("15px");
        new KpiToolTip(marginRight, "Page Margin Right");
        marginBottom = new TextBox();
        marginBottom.setPlaceHolder("-40px");
        new KpiToolTip(marginBottom, "Page Margin Bottom");
        marginLeft = new TextBox();
        marginLeft.setPlaceHolder("90px");
        new KpiToolTip(marginLeft, "Page Margin Left");

        headerHeight = new TextBox();
        headerHeight.setPlaceHolder("300px");
        new KpiToolTip(headerHeight, "Page Header Height");
        footerHeight = new TextBox();
        footerHeight.setPlaceHolder("240px");
        new KpiToolTip(footerHeight, "Page Footer Height");

        InputGroup pageFormatInputGroup = new InputGroup(pageFormat, orientation, headerHeight, footerHeight);
        InputGroup pageMarginInputGroup = new InputGroup(marginTop, marginRight, marginBottom, marginLeft);

        addTitleField(CustomFormConstants.INFORMATION, wfmStrings.information());
        addField(CustomFormConstants.NAME, name, getTitle(wfmStrings.template(), true));
        addField(CustomFormConstants.TYPE, type, getTitle(backendStrings.pdfType(), true));
        ...
        addField(CustomFormConstants.HEADER, header, getTitle(wfmStrings.header()));
        addField(CustomFormConstants.CONTENT, body, getTitle(wfmStrings.content(), true));
        addField(CustomFormConstants.FOOTER, footer, getTitle(wfmStrings.footer()));
        addField(CustomFormConstants.FIELDS, fields, getTitle(wfmStrings.additionalFields()));
        addField(CustomFormConstants.PAGE_FORMAT, pageFormatInputGroup, getTitle(wfmStrings.pageFormat()));
        addField(CustomFormConstants.PAGE_MARGIN, pageMarginInputGroup, getTitle(wfmStrings.pageMargin()));
    ...
    protected void getDataToFillFields() {
        ...
        BackendService.App.get().getCompanyPDFSettings(companyId, objectId, new AbstractAsyncCallback<PDFSettingsTransObject>() {
            ...
            @Override
            public void success(PDFSettingsTransObject result) {
                LoadingPanel.loading(false);

                name.setText(result.getTemplateName());
                type.setItems(asList(result.getReferences()));
                if (result.getPdfReferenceID() != null) {
                    type.setSelected(result.getPdfReferenceID());
                }
                ...
                if (result.getHeader() != null) {
                    header.setText(result.getHeader());
                }
                if (result.getContent() != null) {
                    body.setText(result.getContent());
                }
                if (result.getFooter() != null) {
                    footer.setText(result.getFooter());
                }
                if (result.getPageFormat() != null) {
                    pageFormat.setSelectedByValue(result.getPageFormat());
                }
                if (result.getOrientation() != null) {
                    orientation.setSelectedByValue(result.getOrientation());
                }
                marginTop.setText(result.getMarginTop());
                marginRight.setText(result.getMarginRight());
                marginBottom.setText(result.getMarginBottom());
                marginLeft.setText(result.getMarginLeft());
                headerHeight.setText(result.getHeaderHeight());
                footerHeight.setText(result.getFooterHeight());
            }
        });
    }

    private void onChangeTypes() {
        if (type.getSelectedItem() != null && !Utils.isNullOrEmpty(type.getSelectedItem().getName()) && type.getSelectedItem().getName().equals("Custom Form Item")) {
            sections.setVisible(true);
            customFormItemList.setVisible(true);
        } else {
            sections.setVisible(false);
    ...
    private void save(final boolean closeTab) {
        ...
        transObject.setPdfReferenceID(type.getSelectedId());
        transObject.setTemplateName(name.getText());
        transObject.setHeader(header.getText());
        transObject.setContent(body.getText());
        transObject.setFooter(footer.getText());
        ...
        if (pageFormat.getSelectedItem() != null) {
            transObject.setPageFormat(pageFormat.getSelectedItem().getName());
        }
        if (orientation.getSelectedItem() != null) {
            transObject.setOrientation(orientation.getSelectedItem().getName());
        }
        transObject.setMarginTop(marginTop.getText());
        transObject.setMarginRight(marginRight.getText());
        transObject.setMarginBottom(marginBottom.getText());
        transObject.setMarginLeft(marginLeft.getText());
        transObject.setHeaderHeight(headerHeight.getText());
        transObject.setFooterHeight(footerHeight.getText());
        ...
        BackendService.App.get().saveCompanyPdfTemplate(transObject, new AbstractAsyncCallback<Integer>() {
            ...
            public void success(Integer result) {
                objectId = result;
                WfmUiEventsBus.fireWfmUiEvent(WfmUiEventType.ON_PDF_TEMPLATE_SAVED, objectId, CustomisedPhantomPdfSettingsView.this);
    ...
    private boolean validate() {
        ...
        errors += markAsError(type, !Validation.validateWfmDropdown(type));
        errors += markAsError(name, !Validation.validateTextBoxRequired(name));
        errors += markAsError(body, !Validation.validateTextAreaRequired(body));
    ...
    private SelectItem[] getPageFormats() {
        return new SelectItem[]{
                new SelectItem(1, "A3"),
                new SelectItem(2, "A4"),
                new SelectItem(3, "A5"),
                new SelectItem(4, "Legal"),
                new SelectItem(5, "Letter"),
                new SelectItem(6, "Tabloid")
        };
    }
    ...
    private SelectItem[] getPageOrientations() {
        return new SelectItem[]{
                new SelectItem(1, "portrait"),
                new SelectItem(2, "landscape")
        };
    }

    @Override
    protected String getFormID() {
        return LayoutRPC.PHANTOM_PDF_FORM;
    }
    ...
    private void fillFieldBody() {
        ...
        if (type.getSelectedItem() != null && !Utils.isNullOrEmpty(type.getSelectedItem().getName())) {
            switch (type.getSelectedItem().getName()) {
