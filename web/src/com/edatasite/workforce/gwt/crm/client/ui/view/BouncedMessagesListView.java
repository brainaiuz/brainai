 ...
 */
public class BouncedMessagesListView extends BaseListView implements Constants {
    private static final CrmStrings crmStrings = CrmStrings.App.get();

    private ListingPanel<MessageTrackListItem> list;
    private final Integer objectID;

    public BouncedMessagesListView(Integer objectID) {
        super("bounced", wfmStrings.bouncedEmails());
    ...
    private ColumnDefinitionConfig[] drawColumns() {
        ...
        columns[2] = new ColumnDefinitionConfig<MessageTrackListItem, HTML>(wfmStrings.lastName(), MailMessageItem.LASTNAME, 150) {
            @Override
            public HTML getCellValue(MessageTrackListItem item) {
                if (item.getContactType().equals(CrmConstants.TYPE_LEAD_CONTACT)) {
    ...
    protected Widget onInitialize() {
        list = new ListingPanel<>(ListPanelType.BouncedMessageListPanel, drawColumns(), getDataList(), getDesign());
        ...
        list.setExcelListener(clickEvent -> {
            ...
            filterParameter.setMessageStatus("BOUNCED");
    ...
    private ListingPanelDesign getDesign() {
        return new ListingPanelDesign() {
            ...
            public void initDataEmptyTable(ListingEmptyDataInitializer emptyDataTable) {
                DefaultNoItemsMessage message = new DefaultNoItemsMessage(crmStrings.boundelMessCurrentlyMessages());
    ...
    private ListingRequestProvider<MessageTrackListItem> getDataList() {
        return (ListingFilterParameter filterParametrs, ListingCallback<MessageTrackListItem> callback) -> {
            ...
            MassMailService.App.get().getMessageBouncedList(filterParametrs, new AsyncCallback<ListResult<MessageTrackListItem>>() {
                public void onFailure(Throwable throwable) {
                    callback.onFailure(throwable);
                }

                public void onSuccess(ListResult<MessageTrackListItem> list) {
