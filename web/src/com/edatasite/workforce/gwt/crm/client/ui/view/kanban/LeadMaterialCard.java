...
import com.edatasite.workforce.gwt.crm.client.ui.view.ConvertLeadView;
 ...
 */
public class LeadMaterialCard extends Composite implements PermissionConstants {

    interface LeadMaterialCardUiBinder extends UiBinder<Widget, LeadMaterialCard> {
    }

    private static final LeadMaterialCardUiBinder ourUiBinder = GWT.create(LeadMaterialCardUiBinder.class);
...
//                    WfmUiEventsBus.fireWfmUiEvent(WfmUiEventType.ON_NOTE_ADD, note, NotePopup.this);
    ...
    public LeadMaterialCard(ContactListItem kanbanItem) {
        super();
    ...
    public LeadMaterialCard(ContactListItem kanbanItem, Map<String, KanbanItemColumnConfigs> reletedFieldsMap) {
        super();
        this.kanbanItem = kanbanItem;
...
//            if (kanbanItem != null) {
//                SinksContainerFactory.entryPoint.onHistoryChanged("lead|summary/" + kanbanItem.getObjectId() + "/" + (kanbanItem.getCrmAccount() != null ? kanbanItem.getCrmAccount().getObjectId() : ""), kanbanItem.getName(), kanbanItem.getName());
//            }
//        });

        if (reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_NAME.getCode()) != null) {
            if (reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_NAME.getCode()).isSelected()) {
                String val = getRealValueByCode(reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_NAME.getCode()).getRelatedFieldCode(), kanbanItem.getName());
                leadName.setText(val);
                if (reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_NAME.getCode()).getRelatedFieldCode() == null) {
                    leadName.addClickHandler(clickEvent -> {
                        if (kanbanItem != null) {
                            SinksContainerFactory.entryPoint.onHistoryChanged("lead|summary/" + kanbanItem.getObjectId() + "/" + (kanbanItem.getCrmAccount() != null ? kanbanItem.getCrmAccount().getObjectId() : ""), kanbanItem.getName(), kanbanItem.getName());
                        }
        ...
        MaterialLabel company = new MaterialLabel();
        if (reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_INFO.getCode()) != null) {
            if (reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_INFO.getCode()).isSelected()) {
                String inf = kanbanItem.getCrmAccount() != null && !Utils.isNullOrEmpty(kanbanItem.getCrmAccount().getName()) && !kanbanItem.getCrmAccount().getName().trim().equals("") ? kanbanItem.getCrmAccount().getName() : "";
                String infoValue = getRealValueByCode(reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_INFO.getCode()).getRelatedFieldCode(), inf);
                company.setText(infoValue);
                company.setStyleName("wg_canban__entry-company");
            } else {
                company.setText(kanbanItem.getCrmAccount() != null && !Utils.isNullOrEmpty(kanbanItem.getCrmAccount().getName()) ? kanbanItem.getCrmAccount().getName() : "");
            }
            info2.add(company);
        } else {
        ...
        if (reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_ENTRY_PHOTO.getCode()) != null && reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_ENTRY_PHOTO.getCode()).isSelected()) {
            if (kanbanItem.getContactImageUrl() != null && !kanbanItem.getContactImageUrl().trim().equals("")) {
                MaterialImage photo = new MaterialImage(kanbanItem.getContactImageUrl());
                photo.setWidth("40px");
                photo.setHeight("40px");
                photo.setBorder("0");
                photo.setCircle(true);
                entry_photo.add(photo);
            } else if (kanbanItem.getLeadAssignee() != null) {
                MaterialLabel initials = new MaterialLabel(Utils.getFirstTwoLetters(kanbanItem.getLeadAssignee()));
        ...
        //calls
        boolean isVisiblePhone = reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_PHONE.getCode()) != null && reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_PHONE.getCode()).isSelected();
...
//                    Info.show("Dialing...");
//                    crmService.switchvoxCall(kanbanItem.getPrimaryPhone(), new AbstractAsyncCallback<Void>() {
//                        @Override
//                        public void success(Void result) {
//                        }
//                    });
//                });
//            } else if (Utils.hasTwilioAccess(GenericSettingsEnum.ENABLE_TWILIO)) {
//                callLink.addClickHandler(event -> {
//                    TwilioHelper.initiate("call", kanbanItem.getObjectId(), kanbanItem.isLeadContact() ? RelationItem.TYPE_LEAD : RelationItem.TYPE_CONTACT, kanbanItem.getPrimaryPhone());
//                });
//            } else {
//                callLink.setHref("callto://" + kanbanItem.getPrimaryPhone());
//            }
                ...
                callLink.addClickHandler(clickEvent -> {
                    if (Utils.hasPermission(PermissionConstants.CRM_ADD_NEW_ACTIVITY_LOG_A_CALL) && !Utils.isNullOrEmpty(kanbanItem.getPrimaryPhone())) {
                        ...
                        String relationType = kanbanItem.isLeadContact() ? RelationItem.TYPE_LEAD : RelationItem.TYPE_CONTACT;
                        String relationName = kanbanItem != null ? kanbanItem.getRelationName() : null;

                        Integer finalRelationId = relationId;
                        new ActivityQuickAddForm(Appointment.CALL_LOG, kanbanItem.getPrimaryPhone(), kanbanItem, RelationItem.newEventRelation(relationType, finalRelationId, relationName));
        ...
        boolean isVisibleEmail = reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_EMAIL.getCode()) != null && reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_EMAIL.getCode()).isSelected();
            ...
            if (Utils.hasPermission(CRM_MESSAGE_CENTER) && !Utils.isNullOrEmpty(kanbanItem.getPrimaryEmail())) {
                ...
                emailLink.addClickHandler(clickEvent -> {
                    if (!kanbanItem.isEmailOptOut()) {
                        //new ComposeView(kanbanItem.getPrimaryEmail(), RelationItem.newEventRelation(RelationItem.TYPE_LEAD, kanbanItem.getObjectId(), kanbanItem.getName()));
                        SinksContainerFactory.entryPoint.onHistoryChanged("emailcompose|add/add/" + kanbanItem.getPrimaryEmail() + "/" + RelationItem.TYPE_LEAD + "/" + kanbanItem.getObjectId() + "/" + kanbanItem.getName());
                    } else {
        ...
        //Footer Of Card
        if (reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_ACTION.getCode()) != null && reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_ACTION.getCode()).isSelected()) {
            initKanbanActions(kanbanItem);
        }

        //Notes
        if (reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_NOTE.getCode()) != null && reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_NOTE.getCode()).isSelected()) {
            initNotesIcon();
        }
        ...
        //Assignee
        if (reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_ASSIGNE_NAME.getCode()) != null && reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_ASSIGNE_NAME.getCode()).isSelected()) {
            String assigneeName = getRealValueByCode(reletedFieldsMap.get(KanbanItemSettingEnum.LEAD_ASSIGNE_NAME.getCode()).getRelatedFieldCode(), kanbanItem.getLeadAssignee());
            leadAssigneeName.setText(assigneeName);
        } else {
...
//        actionsLink.ensureDebugId("leadcardaction_"+kanbanItem.getObjectId() );
        actionsLink.setId("leadcardaction_" + kanbanItem.getObjectId());
        ...
        actionsLink.addClickHandler(clickEvent -> {
            ...
            summaryLink.ensureDebugId("lead-view-" + kanbanItem.getObjectId());
            summaryLink.addClickHandler(clickEvent1 -> {
                if (kanbanItem != null) {
                    SinksContainerFactory.entryPoint.onHistoryChanged("lead|summary/" + kanbanItem.getObjectId() + "/" + (kanbanItem.getCrmAccount() != null ? kanbanItem.getCrmAccount().getObjectId() : ""), kanbanItem.getLastName());
                }
            });
            ...
            if (Utils.hasPermission(CRM_LEAD_EDIT)) {
                ...
                editLink.ensureDebugId("lead-edit-" + kanbanItem.getObjectId());
                editLink.addClickHandler(clickEvent12 -> {
                    if (kanbanItem != null) {
                        SinksContainerFactory.entryPoint.onHistoryChanged("leadedit|editlead/" + kanbanItem.getObjectId() + "/" + (kanbanItem.getCrmAccount() != null ? kanbanItem.getCrmAccount().getObjectId() : ""), kanbanItem.getLastName());
                    }
                });
            ...
            if (Utils.hasPermission(PermissionConstants.CRM_LEAD_CONVERT)) {
                MaterialLink convertLink = new MaterialLink(wfmStrings.convert());
                            ...
                            public void onSubmit() {
                                ...
                                contactService.deleteContacts(ids, null, false, new AbstractAsyncCallback<ArrayList<Integer>>() {
                                    ...
                                    public void success(ArrayList<Integer> result) {
                                        Info.show(Property.get(Constants.LEADS, wfmStrings.messSuccessfulyyDeleted(), wfmStrings.lead()), Info.Type.INFO);
                                        WfmUiEventsBus.fireWfmUiEvent(WfmUiEventType.ON_LEADS_DELETE, result, LeadMaterialCard.this);
                                    }
                                });
                            }
                        });
...
//        wg_canban__dropdown_checklist.add(wg_canban__dropdown_checklist_content);
            ...
            addNoteLabel.addClickHandler(clickEvent1 -> new NotePopup(kanbanItem.getObjectId(), RelationItem.TYPE_LEAD));
    ...
    private void initKanbanActivities(final ContactListItem kanbanItem) {
        ...
        activitiesLink.addClickHandler(clickEvent -> {
            ...
            crmService.getLastActivities(kanbanItem.getObjectId(), RelationItem.TYPE_LEAD, new AbstractAsyncCallback<ListResult<Appointment>>() {
                @Override
            ...
            /*MaterialPanel wg_canban__dropdown = new MaterialPanel("wg_canban__dropdown");
            MaterialPanel wg_canban__dropdown_header = new MaterialPanel("wg_canban__dropdown-header");
            MaterialPanel wg_canban__dropdown_notes_icon = new MaterialPanel("wg_canban__dropdown-activities");
//              wg_canban__dropdown_notes_icon.add(new MaterialPanel("ficon--tag-down"));
            wg_canban__dropdown_header.add(wg_canban__dropdown_notes_icon);
            wg_canban__dropdown.add(wg_canban__dropdown_header);
...
//        wg_canban__dropdown_checklist.add(wg_canban__dropdown_checklist_content);
            ...
            //Content (Last note text)
            MaterialPanel wg_canban__dropdown_title = new MaterialPanel("wg_canban__dropdown-title");
            wg_canban__dropdown_title.getElement().setInnerText("Open activities");
            MaterialPanel wg_canban__dropdown_text = new MaterialPanel("wg_canban__dropdown-text");
            wg_canban__dropdown_text.getElement().setInnerText(kanbanItem.getStatus());
            MaterialPanel wg_canban__dropdown_content = new MaterialPanel("wg_canban__dropdown-content");
            wg_canban__dropdown_content.add(wg_canban__dropdown_title);
            wg_canban__dropdown_content.add(wg_canban__dropdown_text);
            ...
            //Add Task
            MaterialLink addTask = new MaterialLink();
            addTask.setStyleName("elm_btn elm_btn--add");
            ...
            MaterialLabel addTaskLabel = new MaterialLabel(crmStrings.addTask());
            addTaskLabel.setStyleName("cp_btn-list-item-title");
            MaterialPanel listItemAddTask = new MaterialPanel("cp_btn-list-item");
            listItemAddTask.add(addTask);
            listItemAddTask.add(addTaskLabel);

            //Add Event
            MaterialLink addEvent = new MaterialLink();
            addEvent.setStyleName("elm_btn elm_btn--add");
            addEvent.addClickHandler(new ClickHandler() {
                @Override
                public void onClick(ClickEvent clickEvent) {
                    if (kanbanItem != null) {
                        new DetailedAppointmentView(null, Appointment.EVENT, RelationItem.newEventRelation(RelationItem.TYPE_LEAD,
            ...
            MaterialLabel addEventLabel = new MaterialLabel(crmStrings.addEvent());
            addEventLabel.setStyleName("cp_btn-list-item-title");
            MaterialPanel listItemAddEvent = new MaterialPanel("cp_btn-list-item");
            listItemAddEvent.add(addEvent);
            listItemAddEvent.add(addEventLabel);


            MaterialPanel cp_btn_list = new MaterialPanel("cp_btn-list");
            cp_btn_list.add(listItemAddTask);
            cp_btn_list.add(listItemAddEvent);
            MaterialPanel wg_canban__dropdown_footer = new MaterialPanel("wg_canban__dropdown-footer");
            wg_canban__dropdown_footer.add(cp_btn_list);
    ...
    private String getRealValueByCode(String fieldCode, String defaultValue) {
        ...
        String result = "";
        if (KanbanItemSettingEnum.LEAD_ASSIGNE_NAME.getCode().equals(fieldCode)) {
            result = kanbanItem.getLeadAssignee();
        } else if (KanbanItemSettingEnum.LEAD_INFO.getCode().equals(fieldCode)) {
            result = kanbanItem.getCrmAccount().getName();
        } else if (KanbanItemSettingEnum.LEAD_NAME.getCode().equals(fieldCode)) {
