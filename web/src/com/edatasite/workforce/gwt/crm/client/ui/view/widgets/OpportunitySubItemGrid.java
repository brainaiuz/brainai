 ...
 */
public class OpportunitySubItemGrid extends AbstractDataGrid<OpportunityItem> {
    ...
    public OpportunitySubItemGrid(Integer opportunityID) {
        super();
        ...
        ItemTableSettingService.App.get().getColumnConfigs(ItemTableEnum.OPPORTUNITY_SUB_ITEM, new AbstractAsyncCallback<ColumnConfigs[]>() {
            ...
            public void onSuccess(ColumnConfigs[] result) {
                OpportunitySubItemGrid.this.columnConfigs = result;
                initialize();
            }
        });
    }
        ...
        if (columnConfigs != null) {
            for (ColumnConfigs cc : columnConfigs) {
                if (ItemTableConstants.PRODUCT.equals(cc.getCode())) {
                    Column<OpportunityItem, String> product = new Column<OpportunityItem, String>(new com.edatasite.workforce.gwt.core.client.ui.cell.ClickableProductLinkCell()) {
                        @Override
                        public String getValue(OpportunityItem item) {
                            String number = item.getItemNumber() != null ? item.getItemNumber() : "";
                            String name = item.getItemName() != null ? item.getItemName() : "";
                            String productId = item.getItemID() != null ? item.getItemID().toString() : "";
                            return number + " -> " + name + "|" + productId;
                        }
                    };
                    ...
                    setColumnWidth(description, cc.getWidth() + "%");
                } else if (ItemTableConstants.DISCOUNT_AMT.equals(cc.getCode())) {
                    Column<OpportunityItem, String> discount = new Column<OpportunityItem, String>(new TextCell()) {
                        @Override
                        public String getValue(OpportunityItem item) {
                            if (item.getDiscountAmount().intValue() > 0) {
                                return AccountingUtils.get().formatDiscount(item.getDiscountAmount()) + (item.getCurrency() != null ? " " + item.getCurrency() : "");
                            } else if (item.getDiscountPercent() != null) {
                                return AccountingUtils.get().formatDiscount(item.getDiscountPercent()) + " %";
                            } else {
                                return AccountingUtils.get().formatDiscount(BigDecimal.ZERO);
                            }
                        }
                    };
                    addColumn(discount, cc.isChanged() ? cc.getTitle() : wfmStrings.discount());
                    setColumnWidth(discount, cc.getWidth() + "%");
                ...
                } else if (ItemTableConstants.CLIENT.equals(cc.getCode())) {
                ...
                } else if (ItemTableConstants.PROJECT.equals(cc.getCode())) {
                    Column<OpportunityItem, String> project = new Column<OpportunityItem, String>(new TextCell()) {
                        @Override
                        public String getValue(OpportunityItem item) {
                            return item.getProject() != null ? item.getProject().getName() : "";
                        }
                    };
                    addColumn(project, cc.isChanged() ? cc.getTitle() : Property.get(Constants.PROJECT, wfmStrings.project()));
                    setColumnWidth(project, cc.getWidth() + "%");
                } else {
    ...
    public void refresher() {
        CRMService.App.get().getOpportunityItems(opportunityID, new AbstractAsyncCallback<OpportunityItem[]>() {
            @Override
            public void onFailure(Throwable caught) {
            }
