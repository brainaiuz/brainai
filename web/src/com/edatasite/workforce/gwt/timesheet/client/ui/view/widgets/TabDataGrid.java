package com.edatasite.workforce.gwt.timesheet.client.ui.view.widgets;
...
/**
 * Created by dilsh0d on 19.08.15.
 */
public class TabDataGrid<T> extends DataGrid<T> {
    ...
    public TabDataGrid(int pageSize) {
        super(pageSize);
        this.addHandler(tabHandler, KeyDownEvent.getType());
    }

    public TabDataGrid(int pageSize, ProvidesKey keyProvider) {
        super(pageSize, keyProvider);
        this.addHandler(tabHandler, KeyDownEvent.getType());
    }

    public TabDataGrid(int pageSize, Resources resources) {
        super(pageSize, resources);
        this.addHandler(tabHandler, KeyDownEvent.getType());
    }

    public TabDataGrid(int pageSize, Resources resources, ProvidesKey keyProvider) {
        super(pageSize, resources, keyProvider);
        this.addHandler(tabHandler, KeyDownEvent.getType());
    }

    public TabDataGrid(int pageSize, Resources resources, ProvidesKey keyProvider, Widget loadingIndicator) {
        super(pageSize, resources, keyProvider, loadingIndicator);
        this.addHandler(tabHandler, KeyDownEvent.getType());
    }

    public TabDataGrid(ProvidesKey keyProvider) {
        super(keyProvider);
        this.addHandler(tabHandler, KeyDownEvent.getType());
    }
    ...
    protected KeyDownHandler tabHandler = new KeyDownHandler() {
        ...
        private void setFocus(Element td, final String dir, KeyDownEvent event) {
            ...
            if (input != null
                    && (input.getTagName().toLowerCase().equals("input")
                    || input.getTagName().toLowerCase().equals("select")
                    || input.getTagName().toLowerCase().equals("button"))) {
                input.focus();
            } else {
                /**
                 ...
                 */
                if (dir!=null && "forward".equals(dir.toLowerCase())) {
        ...
        private Element getParentTD(Element e) {
            ...
            if (parent.getTagName().toLowerCase().equals("td")) {
                return parent;
            }
