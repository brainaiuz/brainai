...
public class KpiHorizontalBarChart extends AbstractChart {

//    private String chartTypeName = Series.Type.BAR.name();

    public KpiHorizontalBarChart(ChartData chartData, DashboardComponentItem dashboardItem) {
        super(chartData, dashboardItem);
//        chartTypeName = super.getChartTypeName();
    }
    ...
    protected void onInitialize() {
        ...
        String maxCategory = chartData.getCategories().stream().max(Comparator.comparingInt(String::length)).get();
        double offset;
        if (maxCategory.contains("...")) {
            offset = 6 * maxCategory.length();
        } else {
            offset = (6 * (maxCategory.length() + 1));
        }

        getXAxis()
                .setCategories(chartData.getCategories().toArray(new String[]{}))
        ...
        // Legend Fix: Highcharts defaults to its base palette for legends.
        ...
        if (chartConf.isShowPieChart()) {
            ...
            // Default to Top
            String vPos = "22%";
            if ("MID".equals(chartConf.getPieChartPosition())) {
                vPos = "50%";
            } else if ("BOTTOM".equals(chartConf.getPieChartPosition())) {
                vPos = "78%";
            }

// X stays locked safely in the right sideroom (82%), Y changes based on user selection
            pieSeries.setOption("center", new String[]{"82%", vPos});
    ...
    protected void setChart3DOption() {
        if (is3DViewOption()) {
            setOptions3D(new Options3D().setEnabled(true).setAlpha(15));
            setOption("/chart/type", "bar");
            if ("LINE_STACKING".equals(selectedOptionType)) {
