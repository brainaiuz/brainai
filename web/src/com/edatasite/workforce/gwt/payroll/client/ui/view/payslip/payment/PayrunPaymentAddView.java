...
public class PayrunPaymentAddView extends FooteredCustomForm implements Constants, Colapse, FittedContent {
    ...
    protected Map<Integer, PayrunPaymentItem> itemMap = new HashMap<>();
    private final HashMap<Integer, PayrunPaymentItem> changedItemMap = new HashMap<>();
    private final HashMap<Integer, Boolean> deletedItemMap = new HashMap<>();
    protected PayrunPayment paymentObject;

    public PayrunPaymentAddView(Integer payrunID) {
        this("add");
        this.payrunID = payrunID;
        setDescription(wfmStrings.payment());
    }
    protected PayrunPaymentAddView(String name) {
        super(name);
    }
    ...
    protected void loadData() {
        ...
        payrollService.initPayrunPayment(filterParameter, new AsyncCallback<PayrunPayment>() {
            @Override
    ...
    protected void setTableData(PayrunPayment result) {
        itemMap.clear();
        employeeTable.removeAllRows();
        for (PayrunPaymentItem item : result.getItems()) {
            employeeTable.addRow(getWidgets(item));
        }
    }

    protected void saveTableItem(Integer id, Object value, String columnID) {
        PayrunPaymentItem item = changedItemMap.get(id);
        if (item == null) {
            item = itemMap.get(id);
        }
    ...
    protected void deleteTableItem(Integer rowId) {
        ...
        PayrunPaymentItem item = itemMap.get(employeeID);
    ...
    protected ListingFilterParameter getFilterParameter() {
        ...
        filterParameter.setGroupPayrunID(payrunID);
    ...
    private void save() {
        ...
        PayrunPayment item = getTransferObject();

        LoadingPanel.loading(true);
        payrollService.createPayrunPayment(item, new AbstractAsyncCallback<SaveResultTO<Integer>>() {
            @Override
            ...
            public void onSuccess(SaveResultTO<Integer> result) {
                ...
                if (result.getResult() != null) {
                    WfmUiEventsBus.fireWfmUiEvent(WfmUiEventType.ON_PAYRUN_PAYMENT_ADD, null, PayrunPaymentAddView.this);
                    closeTab();
    ...
    private PayrunPayment getTransferObject() {
        PayrunPayment item = new PayrunPayment();
        item.setGroupPayrunID(payrunID);
        ...
        item.setTotalItems(totalTableItems);
        return item;
    }
    ...
    void setPaginationData(PayrunPayment result) {
    ...
    protected String getFormID() {
        return LayoutRPC.PAYROLL_PAYRUN_PAYMENT_FORM;
    }
