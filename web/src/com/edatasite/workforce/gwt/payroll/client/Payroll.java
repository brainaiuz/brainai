...
import com.edatasite.workforce.gwt.payroll.client.factory.PayrollSinksContainerFactory;
 ...
 */
public class Payroll extends WorkforceEntryPoint {
    public static List<SelectItem> dashboards = new ArrayList<>();
    public static String defaultDashboardName;

    public interface PayrollStyleResource extends ClientBundle {
        @CssResource.NotStrict
        @Source("com/edatasite/workforce/gwt/payroll/client/Payroll.css")
        CssResource payroll();

        @CssResource.NotStrict
        @Source("com/edatasite/workforce/gwt/payroll/client/PayrollRU.css")
        CssResource payrollRU();

        @CssResource.NotStrict
        @ClientBundle.Source("com/edatasite/workforce/gwt/payroll/client/payrollwelcome/PayrollWelcome.css")
        CssResource payrollWelcome();

        @CssResource.NotStrict
        @ClientBundle.Source("com/edatasite/workforce/gwt/payroll/client/payrollwelcome/PayrollWelcomeRU.css")
        CssResource payrollWelcomeRU();

        @CssResource.NotStrict
        @ClientBundle.Source("com/edatasite/workforce/gwt/payroll/client/payrollwelcome/PayrollWelcomeAR.css")
        CssResource payrollWelcomeAR();

        @CssResource.NotStrict
        @ClientBundle.Source("com/edatasite/workforce/gwt/payroll/client/payrollwelcome/PayrollWelcomeNL.css")
        CssResource payrollWelcomeNL();

        @CssResource.NotStrict
        @ClientBundle.Source("com/edatasite/workforce/gwt/payroll/client/payrollwelcome/PayrollWelcomeTH.css")
        CssResource payrollWelcomeTH();
    }
    ...
    protected void loadUserDashboards() {
        ...
        fp.setModule(ModuleEnum.PAYROLL.getCode());
        fp.setLimit(50);
        ModuleDashboardService.App.get().getModuleDashboards(fp, new AsyncCallback<ArrayList<SelectItem>>() {
            @Override
    ...
    public static PayrollStyleResource styleResource = (PayrollStyleResource) GWT.create(PayrollStyleResource.class);

    public void initSinksContainerFactory() {
        if (Utils.hasPermission(PermissionConstants.PAYROLL_MAIN_MENU)) {
            containerFactory = new PayrollSinksContainerFactory(this);
        } else {
        ...
        styleResource.payroll().ensureInjected();
        String language = Utils.userSettings.get(LANGUAGE_FOR_USER);
        if (language != null) {
            if (language.contains("ru")) {
                styleResource.payrollWelcomeRU().ensureInjected();
                styleResource.payrollRU().ensureInjected();
            } else if (language.contains("ar")) {
                styleResource.payrollWelcomeAR().ensureInjected();
                styleResource.payroll().ensureInjected();
            } else if (language.contains("nl")) {
                styleResource.payrollWelcomeNL().ensureInjected();
                styleResource.payroll().ensureInjected();
            } else if (language.contains("th")) {
                styleResource.payrollWelcomeTH().ensureInjected();
                styleResource.payroll().ensureInjected();
            } else {
                styleResource.payrollWelcome().ensureInjected();
                styleResource.payroll().ensureInjected();
            }
        } else {
            styleResource.payrollWelcome().ensureInjected();
