...
import static com.edatasite.workforce.gwt.core.client.rpc.form.CustomFormConstants.ZONE;

public class WageRateListView extends BaseListView implements Constants {
    ...
    public WageRateListView() {
        super(WAGE_RATE_LIST, settingStrings.wageRate());
    }

    protected Widget onInitialize() {
        listingPanel = new ListingPanel<>(ListPanelType.WageRateList, drawColumns(), getProvider(), getDesigner());
        WfmUiEventsBus.addWfmUiListener(WfmUiEventType.ON_WAGE_RATE_ADD, WageRateListView.this, (sender, args) -> listingPanel.reloadPage());
        add(listingPanel);
        return null;
    }
    ...
    private CustomColumnDefinitionConfig[] drawColumns() {
        ...
        column = new ColumnDefinitionConfig<SelectItem, String>(payrollStrings.payrollZones(), ZONE, LISTING_ACTION.COLUMN_WIDTH) {
            @Override
            public String getCellValue(SelectItem item) {
    ...
    private Anchor getButtonForRow(SelectItem item) {
        ...
        edit.ensureDebugId("wage_rate_edit");
        edit.setCommand(() -> SinksContainerFactory.entryPoint.onHistoryChanged("wageRate|add/" + item.getId(), item.getName()));
        actionItemCount++;
        menuBar.addItem(edit);

        MenuPopItem del = new MenuPopItem(wfmStrings.delete(), "icon-remove");
        del.ensureDebugId("wage_rate_delete");
        del.setCommand(() -> {
            ...
            message.addCloseHandler(new CloseHandler() {
                ...
                public void onSubmit() {
                    ProfileService.App.get().deleteWageRate(item.getId(), new AbstractAsyncCallback<Void>() {
                        public void failure(Throwable caught) {

                        }

                        public void success(Void result) {
                            Info.show(Utils.textFormat(wfmStrings.messSuccessfulyyDeleted(), settingStrings.wageRate()), Info.Type.INFO);
    ...
    private ListingRequestProvider<SelectItem> getProvider() {
        return (filterParametrs, callback) -> ProfileService.App.get().getWageRates(filterParametrs, new AbstractAsyncCallback<ListResult<SelectItem>>() {
            public void failure(Throwable caught) {
    ...
    private ListingPanelDesign getDesigner() {
        return new ListingPanelDesign() {
            ...
            public ActionButton initTopToolBarNew() {
                ...
                addNew.addClickHandler(clickEvent -> SinksContainerFactory.entryPoint.onHistoryChanged("wageRate|add/add"));
                return addNew;
            }
            ...
            public void initDataEmptyTable(ListingEmptyDataInitializer emptyDataTable) {
                DefaultNoItemsMessage message = new DefaultNoItemsMessage(settingStrings.currentlyThereAreNoWageRates());
