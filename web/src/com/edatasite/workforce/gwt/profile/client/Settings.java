...
import com.edatasite.workforce.gwt.profile.client.factory.SettingsSinksContainerFactory;
...
public class Settings extends WorkforceEntryPoint {

    public interface SettingsResources extends ClientBundle {

        @CssResource.NotStrict
        @Source("com/edatasite/workforce/gwt/invoice/client/Invoice.css")
        CssResource invoice();

        @CssResource.NotStrict
        @Source("com/edatasite/workforce/gwt/accounting/client/Accounting.css")
        CssResource accounting();

        @CssResource.NotStrict
        @Source("com/edatasite/workforce/gwt/profile/client/ProfileSettings.css")
        CssResource settings();

    }

    public static SettingsResources resource = GWT.create(SettingsResources.class);


    public void initSinksContainerFactory() {
        if (Utils.hasPermission(PermissionConstants.SETTINGS_MAIN_MENU)) {
            containerFactory = new SettingsSinksContainerFactory(this);
        } else {
        ...
        resource.invoice().ensureInjected();
        resource.accounting().ensureInjected();
        resource.settings().ensureInjected();
    }
