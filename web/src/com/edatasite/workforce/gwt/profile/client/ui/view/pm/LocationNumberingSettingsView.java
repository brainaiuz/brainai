...
public class LocationNumberingSettingsView extends PMDefaultNumberingSettings {
    public LocationNumberingSettingsView() {
    }
    ...
    protected void moveColumnUserPutAll() {
        moveColumnUserPutForLocationNumber();
    }

    @Override
    protected void setDefaultParameters() {
        parseAndSetData(null, "", "L", prefixN, startNumber1, startNumber2, startNumber3, startNumber4, "location");
    }

    @Override
    protected void setSettingsData() {
        parseAndSetData((settings != null ? settings.getLocationNumberingFormat() : null), (settings != null ? settings.getDelimetrLocationNumbering() : ""), "L", prefixN, startNumber1, startNumber2, startNumber3, startNumber4, "location");
    }

    @Override
    protected void setSettings(PMNumberingSettings numberingSettings) {
        super.setSettings(numberingSettings);
        numberingSettings.setLocationNumberingFormat(moveColumnUserPutForLocationNumber());
        numberingSettings.setDelimetrLocationNumbering(delimiterN.getText());
    }

    protected String moveColumnUserPutForLocationNumber() {
        ...
        if (!"".equals(delimiterN.getText())) {
            for (int i = 0; i < horizontalPanel.getWidgetCount(); i++) {
                ...
                if (!"".equals(s.get(0))) {
                    buffer.append(s.get(0)).append(delimiterN.getText());
                }
                bufferFormula.append(s.get(1)).append("/");
            }
            previewText = buffer.substring(0, buffer.toString().length() - 1);
        } else {
            for (int i = 0; i < horizontalPanel.getWidgetCount(); i++) {
                ArrayList<String> s = getPreviewTextEmployee(horizontalPanel.getWidget(i).getElement().getId());
                if (!"".equals(s.get(0))) {
