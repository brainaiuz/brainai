...
public class MyCallsSettingsListView extends BaseListView implements Constants {
    ...
    private ListingPanel<MyCallsSettings> list;

    public MyCallsSettingsListView() {
        super(MY_CALLS_SETTINGS_LIST, "My Calls");
    }

    protected Widget onInitialize() {
        list = new ListingPanel<>(ListPanelType.MyCallsSettingListPanel, getColumnConfigs(), getListData(), getDisagn());
        WfmUiEventsBus.addWfmUiListener(WfmUiEventType.ON_MY_CALLS_SETTINGS_ADD_EDIT, MyCallsSettingsListView.this, (sender, args) -> list.reloadPage());
        add(list);
        return null;
    }
    ...
    private ColumnDefinitionConfig[] getColumnConfigs() {
        ...
        //////////////////////////---------(0)----------
        columns[0] = new ColumnDefinitionConfig<MyCallsSettings, Anchor>(wfmStrings.action(), LISTING_ACTION.COLUMN_CODE, LISTING_ACTION.COLUMN_WIDTH) {
            @Override
            public Anchor getCellValue(final MyCallsSettings item) {
                int actionItemCount = 0;
                MenuBar menuBar = new MenuBar(true);

                MenuPopItem edit = new MenuPopItem(wfmStrings.edit(), "icon-edit");
                edit.getElement().setId("Twilio_setting_edit_button");
                edit.setCommand(() -> SinksContainerFactory.entryPoint.onHistoryChanged("myCallsSetting|add/add/" + item.getObjectID()));
                actionItemCount++;
                ...
                MenuItem.setCommand(() -> {
                    ...
                    messageBox.setMessage(profileMessages.messAreDeleteSMSAccount(item.getUserLogin()));
                    messageBox.addCloseHandler(new CloseHandler() {
                        @Override
                        public void onSubmit() {
                            LoadingPanel.loading(true);
                            service.deleteMyCallsSettings(item.getObjectID(), new AbstractAsyncCallback() {
                                @Override
                                ...
                                public void onSuccess(Object result) {
                                    ...
                                    WfmUiEventsBus.fireWfmUiEvent(WfmUiEventType.ON_ASTERISK_SETTINGS_ADD_EDIT, result, MyCallsSettingsListView.this);
                                }
                            });
                        }
                    });
        ...
        //////////////////////////---------(1)----------
        columns[1] = new ColumnDefinitionConfig<MyCallsSettings, String>(wfmStrings.account(), AsteriskSettings.NUMBER, 100) {
            @Override
            public String getCellValue(MyCallsSettings item) {
                return item.getUserLogin();
            }
        };
        columns[1].setMinimumColumnWidth(40);

        columns[2] = new ColumnDefinitionConfig<MyCallsSettings, String>(wfmStrings.user(), AsteriskSettings.ASTERISK_PORT, 40) {
            @Override
            public String getCellValue(MyCallsSettings item) {
    ...
    private ListingPanelDesign getDisagn() {
        return new ListingPanelDesign() {
            ...
            public ActionButton initTopToolBarNew() {
                ...
                addNew.addClickHandler(clickEvent -> SinksContainerFactory.entryPoint.onHistoryChanged("myCallsSetting|add/add"));
                return addNew;
            }
            ...
            public void initDataEmptyTable(ListingEmptyDataInitializer emptyDataTable) {
                ...
                message.setHref("myCallsSetting|add/add");
//                message.setTextBeforeLink(wfmStrings.addingsipuniAccountClicking());
    ...
    private ListingRequestProvider<MyCallsSettings> getListData() {
        return (filterParametrs, callback) -> {
            filterParametrs = filterParametrs == null ? new ListingFilterParameter() : filterParametrs;
            service.listMyCallsSettings(filterParametrs, new AbstractAsyncCallback<ListResult<MyCallsSettings>>() {
                public void onFailure(Throwable throwable) {
                    callback.onFailure(throwable);
                }

                @Override
                public void onSuccess(ListResult<MyCallsSettings> result) {
