 ...
 * Date: 9/26/12
 */
public class WorksreamSettingsView extends PMDefaultNumberingSettings {

    private String prPreview;

    public WorksreamSettingsView() {
    ...
    protected Widget onInitialize() {
        ...
        getRadioButtons("workstream", settingsStrings.restartNumberingForEachProject(), settingsStrings.useUniqueNumberForWorkstream());

        WfmUiEventsBus.addWfmUiListener(WfmUiEventType.ON_ADD_PROJECT_NUMBER, WorksreamSettingsView.this, (sender, args) -> {
            prPreview = args.toString();
            moveColumnUserPutWorkstream();
        });
    ...
    protected void setSettings(PMNumberingSettings numberingSettings) {
        ...
        numberingSettings.setWorkstreamNumberingFormat(moveColumnUserPutWorkstream());
        numberingSettings.setDelimetrWorkstream(delimiterN.getText());

    }

    @Override
    protected void setDefaultParameters() {
        parseAndSetData(null, "", "W", prefixN, startNumber1, startNumber2, startNumber3, startNumber4, "workstream");
    }

    @Override
    protected void setSettingsData() {
        parseAndSetData((settings != null ? settings.getWorkstreamNumberingFormat() : null), (settings != null ? settings.getDelimetrWorkstream() : ""), "W", prefixN, startNumber1, startNumber2, startNumber3, startNumber4, "workstream");
    }

    protected String moveColumnUserPutWorkstream() {
        ...
        if (!"".equals(delimiterN.getText())) {
            for (int i = 0; i < horizontalPanel.getWidgetCount(); i++) {
                ArrayList<String> isDelimet = getPreviewTextW(horizontalPanel.getWidget(i).getElement().getId());
            ...
            for (int i = 0; i < horizontalPanel.getWidgetCount(); i++) {
                ArrayList<String> noDelimetr = getPreviewTextW(horizontalPanel.getWidget(i).getElement().getId());
    ...
    private ArrayList<String> getPreviewTextW(String wTitle2) {
