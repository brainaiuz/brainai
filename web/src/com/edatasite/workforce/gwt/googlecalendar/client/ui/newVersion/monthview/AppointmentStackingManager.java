...
public class AppointmentStackingManager {
    ...
    public int singleDayLowestOrder(int day) {
        ...
        int currentlyInspectedLayer = 0;
        do {
            boolean isLayerAllocated = isLayerAllocated(currentlyInspectedLayer);
            if (isLayerAllocated) {
    ...
    public ArrayList<AppointmentLayoutDescription> getDescriptionsInLayer(int layer) {
        return layeredDescriptions.get(layer);
    }

    public HashMap<Integer, ArrayList<AppointmentLayoutDescription>> getDescriptions() {
        return layeredDescriptions;
    }

    private boolean overlapsWithDescriptionInLayer(ArrayList<AppointmentLayoutDescription> layerDescriptions, int start, int end) {
        if (layerDescriptions != null) {
            for (AppointmentLayoutDescription layerDescription : layerDescriptions) {
                if (layerDescription.overlapsWithRange(start, end)) {
                    return true;
                }
            }
        }
    ...
    private boolean assignLayer(int layer, AppointmentLayoutDescription description) {
        ArrayList<AppointmentLayoutDescription> layerDescriptions = layeredDescriptions.get(layer);

        boolean assigned = false;
        if (!overlapsWithDescriptionInLayer(layerDescriptions, description.getWeekStartDay(), description.getWeekEndDay())) {
    ...
    private void initLayer(int layerNumber) {
        if (!isLayerAllocated(layerNumber)) {
            layeredDescriptions.put(layerNumber, new ArrayList<>());
        }
    }
    ...
    public boolean areThereAppointmentsOn(int day) {
        ...
        for (int layersIndex = 0; layersIndex <= highestLayer; layersIndex++) {
            ArrayList<AppointmentLayoutDescription> layerDescriptions = layeredDescriptions.get(layersIndex);
