 ...
 */
public class Hrms extends WorkforceEntryPoint {

    public static List<SelectItem> dashboards = new ArrayList<>();
    public static String defaultDashboardName;
    public static HashMap<Integer, OnboardingItem> customSteps = new LinkedHashMap<>();
    ...
    protected void loadUserDashboards() {
        ...
        fp.setModule(ModuleEnum.HRMS.getCode());
        fp.setLimit(50);
    ...
    protected void loadOnboardingCustomSteps() {
        HrmsService.App.get().getOnboardingStepsForListing(new AbstractAsyncCallback<ArrayList<OnboardingItem>>() {
            @Override
            public void onFailure(Throwable caught) {
            ...
            @Override
            public void onSuccess(ArrayList<OnboardingItem> result) {
                if (result != null && result.size() > 0) {
                    for (OnboardingItem item : result) {
                        customSteps.put(item.getStepId(), item);
                    }
                }
                initDefaultUserSettings();
            }
    ...
    public void initSinksContainerFactory() {
        if (Utils.hasPermission(PermissionConstants.HRMS_MAIN_MENU)) {
            containerFactory = new HrmsSinksContainerFactory(this);
        } else {
