 ...
 */
public class AddCompanyDocumentsView extends GeneralDocumentsView implements Colapse {
    private WfmButton2 saveClose;


    public AddCompanyDocumentsView(Integer id, Integer companyID) {
        super("document", wfmStrings.documents(), Constants.F_COMPANY_DOCUMENTS, companyID, companyID, null);
    }
        ...
        if (validate(generalFileUpload)) {
            ...
            DocumentsService.App.get().updateFiles(generalFileUpload.getNewlyAttachedFiles(), entityId, typeCode, new AbstractAsyncCallback<Void>() {
                ...
                public void success(Void v) {
                    LoadingPanel.loading(false);
                    WfmUiEventsBus.fireWfmUiEvent(WfmUiEventType.ON_EMPLOYEE_FILES_UPLOADED, v, AddCompanyDocumentsView.this);
                    closeTab();
                }
            });
        }
