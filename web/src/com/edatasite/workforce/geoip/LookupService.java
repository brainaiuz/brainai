...
package com.edatasite.workforce.geoip;
...
public class LookupService {
    ...
    private synchronized int seekCountry(long ipAddress) {
        ...
        for (int depth = 31; depth >= 0; depth--) {
            if ((dboptions & GEOIP_MEMORY_CACHE) == 1) {
                //read from memory
                System.arraycopy(dbbuffer, (2 * recordLength * offset) + 0, buf, 0, 2 * MAX_RECORD_LENGTH);
            } else if ((dboptions & GEOIP_INDEX_CACHE) != 0) {
                //read from index cache
                System.arraycopy(index_cache, (2 * recordLength * offset) + 0, buf, 0, 2 * MAX_RECORD_LENGTH);
            } else {
                //read from disk
                try {
                    file.seek(2L * recordLength * offset);
                    file.read(buf);
                }
                catch (IOException e) {
                    System.out.println("IO Exception");
