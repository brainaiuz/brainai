...
public final class MessageBeanUtil {
    static final Logger logger = LoggerFactory.getLogger(MessageBeanUtil.class);
    ...
    public static MessageBean mimeToBean(Part p) throws IOException,
        ...
        processAttachment(msgBean, p, msgBean, 0);
    ...
    private static String processEnvelope(Message msg, MessageBean msgBean)
        ...
        try {
            ...
            if ((addr = msg.getFrom()) != null && addr.length > 0) {
                StringBuilder addrStr = new StringBuilder(checkAddr(addr[0].toString()));
                for (int j = 1; j < addr.length; j++) {
                    addrStr.append(",").append(checkAddr(addr[j].toString()));
                }
                from = InternetAddress.parse(addrStr.toString());
                ...
                StringBuilder addrStr = new StringBuilder(checkAddr(_froms[0]));
                for (int j = 1; j < _froms.length; j++) {
                    addrStr.append(",").append(checkAddr(_froms[j]));
                }
                from = InternetAddress.parse(addrStr.toString());
            } else {
        ...
        try {
            if ((addr = msg.getRecipients(RecipientType.TO)) != null && addr.length > 0) {
                ...
                envelope_to = InternetAddress.parse(addrStr.toString());
            } else {
        ...
        try {
            ...
            if (dlvrTo != null && dlvrTo.length > 0) {
                StringBuilder addrStr = new StringBuilder(checkAddr(dlvrTo[0]));
                for (int j = 1; j < dlvrTo.length; j++) {
                    addrStr.append(",").append(checkAddr(dlvrTo[j]));
                }
                //logger.info("processEnvelope() - \"Delivered-To\" found from header: " + addrStr);
                delivered_to = InternetAddress.parse(addrStr.toString());
            }
        }
        catch (MessagingException e) {
        ...
        try {
            if ((addr = msg.getReplyTo()) != null && addr.length > 0) {
                ...
                replyto = InternetAddress.parse(addrStr.toString());
        ...
        try {
            String[] priority = msg.getHeader(XHEADER_PRIORITY);
            if (priority != null) {
    ...
    */
    ...
    private static void setAnodeValue(BodypartBean anode, Part p, String errmsg) {
        try {
            anode.setValue(p.getInputStream());
        }
        catch (Exception e) {
    ...
    private static java.util.Date getHeaderDate(String text) {
        ...
        try {
            return mailDateFormat.parse(text);
        }
    ...
    */
    ...
    public static String getFileName(String ctype) {
        String desc = null;
        if (ctype != null && ctype.contains("name=")) {
    ...
    public static Message beanToMime(MessageBean msgBean)
        ...
        jakarta.mail.Session session = Session.getDefaultInstance(System.getProperties());
        if (debugSession)
        ...
        if (msgBean.getMimeType().startsWith("multipart")) {
            ...
            msg.setContent(mp);
            constructMultiPart(mp, msgBean, 0);
            ...
            if (msgBean.getValue() != null) {
                ...
                mp.addBodyPart(bp);
                constructSinglePart(bp, msgBean, 0);
            }
            constructMultiPart(mp, msgBean, 0);
        } else {
            constructSinglePart(msg, msgBean, 0);
        }
    ...
    private static final Set<String> reservedHeaders = new HashSet<>();
        ...
        for (Method method1 : methods) {
            ...
            if (Modifier.isPublic(mod) && !Modifier.isAbstract(mod) && !Modifier.isStatic(mod)) {
                if (method.getName().length() > 3 && method.getName().startsWith("get")
                    ...
                    } else if (method.getReturnType().getCanonicalName().equals("jakarta.mail.Address[]")) {
                        methodNameList.add(name);
                    }
                }
            }
    ...
    public static void main(String[] args) {
        ...
        for (String aMethodNameList : methodNameList) {
            sb.append(aMethodNameList + LF);
        }
