...
public class BodypartBean implements Serializable {
    ...
    static final Logger logger = LoggerFactory.getLogger(BodypartBean.class);
    ...
    public String toString(int level) {
        ...
        if (!(this instanceof MessageBean)) {
            for (MsgHeader header : headers) {
                MsgHeader hdr = header;
                sb.append("Header Line - " + hdr.getName() + ": " + hdr.getValue() + LF);
            }
        }
        if (value != null) {
            if (getMimeType().contains("text") || getMimeType().contains("message"))
