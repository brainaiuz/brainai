...
public abstract class TableBulkRenderer<RowType> implements
    ...
    protected static class BulkCellView<RowType> extends
         ...
         */
        private Map<String, String> curCellStyles = new HashMap<>();
         ...
         */
        private List<DelayedWidget> delayedWidgets = new ArrayList<>();

        /**
         * The {@link Widget Widgets} that need to be added to the table after it
         * has finished loading.
         */
        private Map<Integer, String> rowColors = new HashMap<>();
        ...
        public void setRowColor(String color) {
            rowColors.put(getRowIndex(), color);
        }
    ...
    protected static class BulkRowView<RowType> extends AbstractRowView<RowType> {
         ...
         * The style attributes to apply to the current cell.
         */
        private Map<String, String> curRowStyles = new HashMap<>();
            ...
            class RenderTableCommand implements IncrementalCommand {
                public boolean execute() {
                    ...
                    while (rowValues.hasNext()) {
                        ...
                        if (!options.syncCall && --checkRow == 0) {
                    ...
                    // Add widgets into the table todo need to optimize this. when the rows more then 100 the listing works slowly
                    ...
                    if (cellView.rowColors.size() > 0) {
                        for (Map.Entry<Integer, String> rowColor : cellView.rowColors.entrySet()) {
                            Element row = bulkRenderer.getTable().getRowFormatter().getElement(rowColor.getKey());
                            if (row != null && rowColor.getValue() != null) {
                                String color = rowColor.getValue();
                                color = color.replace("#", "");
                                int[] colors = Utils.convertHexToRGB(color);
                                DivElement div = Document.get().createDivElement();
                                div.getStyle().setProperty("background", "rgba(" + colors[0] + ", " + colors[1] + ", " + colors[2] + ", 1)");
                                div.addClassName("row-decorator");
                                row.getStyle().setProperty("background", "rgba(" + colors[0] + ", " + colors[1] + ", " + colors[2] + ", 0.1)");
                                row.getFirstChildElement().appendChild(div);
                            }
                        }
                    }
    ...
    public final void renderRows(Iterable<RowType> rows, RendererCallback callback) {
        IterableTableModel<RowType> tableModel = new IterableTableModel<>(
                rows);
    ...
    protected AbstractRowView<RowType> createRowView(
        ...
        BulkCellView<RowType> cellView = new BulkCellView<>(this);
