...
public class SelectionGrid extends Grid implements HasRowHighlightHandlers,
    ...
    public static enum SelectionPolicy {
        ONE_ROW(null), MULTI_ROW(null), CHECKBOX("<span class='cell-decorator'><label class='gwt-control control control--checkbox'><input type='checkbox'/><span class='control__indicator'/></label></span>"), RADIO(
                "<span class='cell-decorator'><div class='selection-grid__radio'><input name='%NAME%' type='radio'/><span class='control__indicator'/></div></span>");
        ...
        switch (DOM.eventGetType(event)) {
            ...
            case Event.ONMOUSEDOWN: {
                ...
                if (selectionPolicy == SelectionPolicy.MULTI_ROW) {
                    ...
                    // Select the rows
                    selectRow(targetRowIndex, ctrlKey, shiftKey, false, "");
                } else if (selectionPolicy == SelectionPolicy.ONE_ROW || (selectionPolicy == SelectionPolicy.RADIO && targetCell == targetRow.getFirstChild())) {
                    selectRow2(-1, targetRow, true, true, false, "");
                    lastSelectedRowIndex = targetRowIndex;
                } else if (selectionPolicy == SelectionPolicy.CHECKBOX && targetCell == targetRow.getFirstChild()) {
                    String className = event.getEventTarget().toString();
                    String eventClassName = className.substring(className.indexOf(" "), className.indexOf("]"));
                    selectRow(targetRowIndex, true, shiftKey, true, eventClassName);
                }
            }
            break;
            ...
            case Event.ONCLICK: {
                ...
                if (selectionPolicy == SelectionPolicy.CHECKBOX
                    ...
                    selectRow(targetRowIndex, true, DOM.eventGetShiftKey(event), true, "");
                }
            }
            break;
        }
    ...
    public void selectAllRows() {
        ...
        for (int i = 0; i < rowCount; i++) {
            if (!selectedRows.containsKey(i)) {
                selectRow2(i, rowFormatter.getElement(i), false, false, true, "");
            }
        }
    ...
    public void selectRow(int row, boolean unselectAll) {
        selectRow2(row, getRowFormatter().getElement(row), unselectAll, true, true, "");
    }
     ...
     * @throws IndexOutOfBoundsException
     */
    public void selectRow(int row, boolean ctrlKey, boolean shiftKey, boolean changeInput, String targetClassName) {
        ...
        if (shiftKey && (lastSelectedRowIndex > -1)) {
            ...
            for (int curRow = firstRow; curRow <= lastRow; curRow++) {
                if (isSelected) {
                    deselectRow(curRow, false, changeInput, "");
                } else {
                    selectRow2(curRow, formatter.getRawElement(curRow), false, false, changeInput, "");
                }
            }
            ...
            deselectRow(row, true, true, targetClassName);
            lastSelectedRowIndex = row;
            fireRowSelectionEvent(oldRowList);
        } else {
            // Select the row
            SelectionGridRowFormatter formatter = getSelectionGridRowFormatter();
            selectRow2(row, formatter.getRawElement(row), false, false, changeInput, targetClassName);
            lastSelectedRowIndex = row;
            fireRowSelectionEvent(oldRowList);
        }
    }
     ...
     */
    protected void deselectAllRows(boolean fireEvent, boolean changeInput) {
        // Get the old list of selected rows
        ...
        for (Element rowElem : selectedRows.values()) {
            ...
            if (changeInput && hasInputColumn) {

                if (((Element) rowElem.getFirstChildElement()).hasChildNodes()) {
                    setInputSelected(getSelectionPolicy(), (Element) rowElem.getFirstChildElement().getFirstChildElement(), false, "");
                } else {
     ...
     * @param fireEvent true to fire events
     */
    protected void deselectRow(int row, boolean fireEvent, boolean changeInput, String targetClassName) {
        ...
        if (rowElem != null) {
            ...
            // Deselect the row
            setStyleName(rowElem, "selected", false);
            if (selectionPolicy.hasInputColumn() && changeInput) {
                setInputSelected(getSelectionPolicy(), (Element) rowElem.getFirstChildElement().getFirstChildElement(), false, targetClassName);
            }
     ...
     * @param targetClassName
     */
    protected void selectRow2(int row, Element rowElem, boolean unselectAll,
                              boolean fireEvent, boolean changeInput, String targetClassName) {
        ...
        if (changeInput && selectionPolicy.hasInputColumn()) {

            if (((Element) rowElem.getFirstChildElement()).hasChildNodes()) {
                setInputSelected(getSelectionPolicy(), (Element) rowElem.getFirstChildElement().getFirstChildElement(), true, targetClassName);
            } else {
                setInputSelected(getSelectionPolicy(), (Element) rowElem.getFirstChildElement(), true, targetClassName);
            }
    ...
    protected void setInputSelected(SelectionPolicy selectionPolicy, Element td,
                                    boolean selected, String targetClassName) {


        if (targetClassName.trim().equals("HTMLSpanElement")) {
            ((InputElement) td.getFirstChild().getFirstChild()).setChecked(selected);
        }
        /*if (targetClassName.trim().equals("HTMLTableCellElement")) {
            ((InputElement) td.getFirstChild().getFirstChild()).setChecked(selected);
        }*/
        if (targetClassName.trim().equals("HTMLInputElement")) {
