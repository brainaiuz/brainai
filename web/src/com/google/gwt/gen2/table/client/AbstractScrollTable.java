...
public abstract class AbstractScrollTable extends ComplexPanel implements
    ...
    protected final WfmStrings wfmStrings = WfmStrings.App.get();
    /**
    ...
    private static class MouseResizeWorker {
        ...
        /**
         * The executes just after the column resizes. (Added by Anvar Akramov (T1528))
         */

        public ExecuteCommand onStopColumnResize = null;
        ...
        public void setOnStopColumnResize(ExecuteCommand onStopColumnResize) {
            this.onStopColumnResize = onStopColumnResize;
        }
...
//                GWT.log("stopResizing ------------------------------- ");
                if(onStopColumnResize!=null) {
                    onStopColumnResize.execute();//Added by Anvar Akramov (T1528)
                }
            }
        }
    ...
    public void addOnStopColumnResize(ExecuteCommand onStopColumnResize) {
        this.resizeWorker.setOnStopColumnResize(onStopColumnResize);
    }
    ...
    public abstract ColumnDefinition<?, ?> getColumnDefinition(int colIndex);
    ...
    protected void applySortedColumnIndicator(Element tdElem, boolean ascending) {
        ...
        tdElem.getFirstChildElement().appendChild(sortedColumnWrapper);
        if (ascending) {
            sortedColumnWrapper.setInnerHTML("&nbsp;"
                    + "<svg class=\"icon--sortAsc\"><use href=\"mainStyles/new-ui/icons/sprite__panels.svg?v="+ Utils.getUploadVersion()+"#sortAsc\"></use></svg>\n");
        } else {
            sortedColumnWrapper.setInnerHTML("&nbsp;"
                    + "<svg class=\"icon--sortDesc\"><use href=\"mainStyles/new-ui/icons/sprite__panels.svg?v="+Utils.getUploadVersion()+"#sortDesc\"></use></svg>\n");
        }
        sortedRowIndex = -1;
        sortedCellIndex = -1;
    ...
    private ColumnWidthInfo getColumnWidthInfo(int column) {
        ...
        //Check Action column width
        ColumnDefinition colDef = getColumnDefinition(column);
        HeaderProperty prop = (HeaderProperty) (colDef.getColumnProperty(HeaderProperty.TYPE));
        String columnName = (String) prop.getHeader(0, column);

//        GWT.log(prop.getHeader(0, column) + " - " + column);
        ...
        //T1528 changes (otherwires you will not be able to resize columns)
        if( (column>0 || !SelectionGrid.SelectionPolicy.CHECKBOX.equals(dataTable.getSelectionPolicy()) || !SelectionGrid.SelectionPolicy.RADIO.equals(dataTable.getSelectionPolicy()) )
                && !(wfmStrings.action().equalsIgnoreCase(columnName)) ) {
            return new ColumnWidthInfo(1, maxWidth, preferredWidth, curWidth);
        } else {
