...
//                    setFocus(true); //reset scroll to the top
                }
                break;
            }
            ...
            case Event.ONKEYUP: {
                if (DOM.eventGetKeyCode(event) == KeyCodes.KEY_TAB) {
                    ArrayList<Element> chain = new ArrayList<>();
    ...
    */
    ...
    private void onSelection(TreeItem item, boolean fireEvents, boolean moveFocus) {
        ...
        if (curSelection != null) {
            if (moveFocus) {
//        moveFocus();      /*it's commented on demand lochin aka*/
            }
    ...
    private void setImages(ImageAdapter images, boolean useLeafImages) {
        ...
        if (!useLeafImages) {
            ...
            int size = image.getWidth() + TreeItem.IMAGE_PAD - 3;//stasni dizayni buyicha o'zgartirildi(By Hayot)
            image.removeFromParent();
            indentValue = (size) + "px";
        }
    }

    public void showImage(TreeItem treeItem, AbstractImagePrototype proto) {
        ...
        if (child == null) {
            ...
            // prototype.
            DOM.appendChild(holder, proto.createElement().cast());
        } else {
            // Otherwise, simply apply the prototype to the existing element.
            proto.applyTo(child.cast());
        }
    }
