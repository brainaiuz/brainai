...
public class TextBox extends TextBoxBase {
    ...
    public static int DEFAULT_MAX_LENGTH = 1000;//default max length = 255; Changed from 255 to 1000
     ...
     */
    public TextBox() {
        this(Document.get().createTextInputElement(), "form-control");
        if (getElement() != null) {
            getElement().setAttribute("autocomplete", "off");
        }
    }

    public TextBox(boolean isAutoCompleteOff) {
        this(Document.get().createTextInputElement(), "form-control");
    ...
    private native void setSelectionRange(Element element, Integer start, Integer end) /*-{
        $wnd.jQuery(element).get(0).setSelectionRange(start, end);
    }-*/;

    protected TextBox(Element element) {
        super(element);
        setMaxLength(DEFAULT_MAX_LENGTH); //default max length = 255;
