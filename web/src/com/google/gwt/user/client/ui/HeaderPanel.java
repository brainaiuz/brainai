...
public class HeaderPanel extends Panel implements RequiresResize {
  ...
  public HeaderPanel() {
    ...
    headerContainer = createContainer();
     //headerContainer.getStyle().setPaddingRight(barWidth, PX);
    headerContainer.getStyle().setTop(0.0, PX);
    headerImpl.init(headerContainer, resizeDelegate);
    elem.appendChild(headerContainer);

    // Create the footer container.
    footerContainer = createContainer();
    footerContainer.getStyle().setBottom(0.0, PX);
    ...
    contentContainer.getStyle().setTop(0.0, PX);
//    contentContainer.getStyle().setHeight(0.0, PX);
  ...
  private Element createContainer() {
    ...
    container.getStyle().setLeft(0.0, PX);
  ...
  private void forceLayout() {
    ...
    if (header != null) {
      ...
      contentContainer.getStyle().setTop(height, PX);
    } else {
      contentContainer.getStyle().setTop(0.0, PX);
    }
    if (footer != null) {
    ...
    if (remainingHeight > 0) {
      contentContainer.getStyle().setHeight(Math.max(0, remainingHeight), PX);
    }
