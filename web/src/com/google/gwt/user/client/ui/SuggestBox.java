...
public class SuggestBox extends Composite implements HasText, HasFocus,
    ...
    public static class DefaultSuggestionDisplay extends SuggestionDisplay
        ...
        protected void showSuggestions(final SuggestBox suggestBox,
            ...
            for (final Suggestion curSuggestion : suggestions) {
                ...
                menuItem.setCommand(() -> callback.onSuggestionSelected(curSuggestion));
    ...
    public static final String STYLE_NAME_DEFAULT = "gwt-SuggestBox";
    ...
    private final Callback callback = (request, response) -> showSuggestions(response.getSuggestions());
    private final SuggestionCallback suggestionCallback = suggestion -> {
//      setNewSelection(suggestion);
    ...
    public SuggestBox(SuggestOracle oracle, TextBoxBase box,
        ...
        setOracle(oracle);
        setStyleName(STYLE_NAME_DEFAULT);
    }
     ...
     * @deprecated use {@link #getTextBox}().addValueChangeHandler instead
     */
    @Deprecated
     ...
     *             object returned by {@link #getTextBox}().addValueChangeHandler
     *             instead
     */
    @Deprecated
            ...
            public void onKeyDown(KeyDownEvent event) {
                switch (event.getNativeKeyCode()) {
                    case KeyCodes.KEY_DOWN:
                        suggestionMenu.selectItem(suggestionMenu.getSelectedItemIndex() + 1);
                        suggestionMenu.getSelectedItem().getElement().scrollIntoView();
                        break;
                    ...
                    case KeyCodes.KEY_TAB:
                        if (suggestionMenu.getSelectedItemIndex() <= 0) {  // this is by Bunyod s
                            suggestionPopup.hide();
                        } else {
    ...
    private void showSuggestions(Collection<? extends SuggestOracle.Suggestion> suggestions) {
        if (suggestions.size() > 0) {
            ...
            suggestionMenu.clearItems();
            boolean containsAddNew = false;
            for (SuggestOracle.Suggestion curSuggestion : suggestions) {
                final SuggestionMenuItem menuItem = new SuggestionMenuItem(
                        curSuggestion, oracle.isDisplayStringHTML());
                menuItem.setCommand(() -> SuggestBox.this.setNewSelection(menuItem));
                if (menuItem.getSuggestion().getDisplayString().startsWith("<a") && menuItem.getSuggestion().getDisplayString().contains("gwt-Anchor")){
                    containsAddNew = true;
                }
                suggestionMenu.addItem(menuItem);
            }
            ...
            if (containsAddNew && suggestionMenu.getNumItems() > 1) {
                // Select the second item in the suggestion menu if add new exists
                suggestionMenu.selectItem(1);
            }
    ...
    public void setZIndex(int i) {
        ...
        if (suggestionMenu != null) {
            getElement().getStyle().setZIndex(i + 10);
        }
        if (suggestionPopup != null) {
            getElement().getStyle().setZIndex(i + 15);
        }
    }
